...
- Product requirement, design system, project status, QA checklist, dan regression contract diselaraskan dengan flow baru tanpa mengubah schema/API/ledger/authorization.
- Flow Pengeluaran generic memakai decision 0/1/banyak kandidat pada **Penggunaan dana**: nol Kebutuhan cocok langsung memakai **Dana Tersedia**, satu kandidat auto-link namun tetap terlihat/editable, dan beberapa kandidat baru meminta pilihan user.
- Konfirmasi dua langkah `Simpan tetap` khusus expense tanpa Alokasi dihapus karena sumber dana sudah terlihat sebelum submit; duplicate confirmation, overspend guard/alasan, period lock, dan transfer approval tetap canonical.
- Copy mobile/desktop diseragamkan menjadi **Dana Tersedia · Tanpa Alokasi** untuk jalur unallocated, sementara entry contextual dari Kebutuhan tetap terkunci dan perubahan rekening/kategori/tanggal menjalankan smart matching ulang.
- Overspend policy `block` tetap dihormati backend; warning sekarang mengarahkan user untuk memilih **Dana Tersedia**, Kebutuhan lain, atau mengurangi nominal.

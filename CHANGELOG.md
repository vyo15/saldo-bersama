...
- Mengembalikan login mobile ke Firebase popup dengan tombol custom Saldo Bersama tanpa mengubah layout login. Module auth dipreload sebelum tombol aktif agar `signInWithPopup` dipanggil langsung dari user gesture, client memakai in-memory persistence, error popup dibatasi ke pesan aman, dan reverse proxy `/__/auth/*` yang hanya dibutuhkan flow redirect dipensiunkan. Desktop GIS dan backend allowlist/session tetap tidak berubah.
- Memperkuat recovery login Google mobile setelah kembali dari redirect: intent session ditandai sebelum keluar ke Google, `getRedirectResult` tetap jalur utama, dan fallback `Auth.authStateReady()` + `currentUser` hanya dipakai untuk intent redirect yang masih valid sebelum ID token diteruskan ke backend session. Flow mobile juga menghapus progress bar/counter serta tombol back visual agar onboarding cukup memakai swipe, pagination, dan Lewati.
- Mengganti login Google mobile dari Firebase popup ke redirect yang direkomendasikan untuk perangkat mobile, dengan `getRedirectResult`, session persistence, reverse proxy same-origin `/__/auth/*` pada Vercel, error canonical aman, serta menghapus tombol onboarding `Lanjut` yang redundan; desktop Google Identity Services, backend allowlist, role, dan verifikasi ID token tidak berubah.

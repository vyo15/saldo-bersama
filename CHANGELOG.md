...
- Holding dapat dialokasikan sebagian ke satu atau beberapa Target selama total alokasi tidak melebihi holding aktual. Sell yang menyentuh porsi Target wajib membawa konteks Target dan dapat **tetap untuk Target** sebagai retained cash atau dilepas; release holding/retained cash divalidasi fail-closed untuk mencegah progress negatif dan double count.
- `Investasi → Beli/Jual` menerima Target opsional. Posisi investasi pertama yang dibuat dari `Target → Tambah dana → Investasi` menautkan `goal_id` dalam transaction backend yang sama, sehingga create posisi dan alokasi Target bersifat atomik dan tidak memiliki celah create-sukses/link-gagal.
- Quick Catat Target memakai route baru `choose-goal-funding` / `goal-fund`; contract `goal-plan`, `goal-deposit`, serta `AllocationGoalExecutionModal` dipensiunkan. Entry dari Target mempertahankan konteks Target sampai Buy/setup Investasi selesai dan close/back tidak menghidupkan workflow yang sudah dikonsumsi.
- Read model, lifecycle, integrity check, backup/restore/reset, notification/reminder, API contract, schema docs, product requirement, design system, serta regression test diselaraskan dengan satu sumber progress baru. Schema v23/flow lama tetap hanya tercatat sebagai histori migration/changelog, bukan contract current.

## 2026-09-19 — UI/UX density audit & desktop consistency
- Menuntaskan audit kepadatan lintas halaman tanpa mengubah authority ledger/authorization: shell desktop memakai satu lebar konten **1320px**, header route diringkas, dan pull-to-refresh hanya dirender pada viewport mobile agar kontrol sentuh tidak bocor ke desktop.
- Cocokkan saldo memberi ruang penuh pada riwayat sebelum breakpoint dua-panel dan proporsi desktop diperlebar untuk data, sehingga kolom penting seperti **Selisih** dan **Status** tidak tersembunyi oleh panel form.
- Ledger Transaksi desktop dirapatkan ke sekitar 56px dan status normal `Aktif` menjadi implisit; Persetujuan kosong kini satu empty-state; ringkasan Kewajiban disembunyikan ketika hanya ada satu item aktif.
- Pengaturan desktop disederhanakan menjadi **Menu terkelompok → Detail**; Anggota menghapus hero dekoratif dan hanya menampilkan search/filter saat dataset >5 atau filter aktif; Jadwal Rutin hanya menampilkan pemilih Pengeluaran/Pemasukan bila kedua jenis tersedia dan menaruhnya sebaris dengan heading.
- Grid Alokasi/Target/Kewajiban menyeimbangkan baris terakhir, Kategori desktop memakai `auto-fit`, dan tab Atur Dana desktop mengikuti lebar isi sementara mobile tetap tiga target sentuh penuh. Surface bersarang di Beranda/Investasi diratakan dan grid Laporan/Beranda tidak lagi meregangkan card pendek secara tidak perlu.
- Dokumentasi dan regression contract diselaraskan dengan struktur final; asset `house.webp` dihapus karena hero Anggota sudah dipensiunkan dan tidak memiliki consumer runtime lagi.

## 2026-09-19 — Quick Catat mobile contextual & compact
- Memperbaiki loop `Maximum update depth exceeded` pada workflow Quick Catat: route kini dikonsumsi **sekali per navigation intent**, state route dibersihkan sebelum modal/action dibuka, dan dependency Target tidak lagi bergantung pada object resource yang berubah referensi setiap render. `Batal`/`×` benar-benar menutup modal tanpa direopen oleh intent lama.
- Guard consume-once diterapkan ke workflow Alokasi, Target, dan Investasi. Contract Target lama `goal-plan`/`goal-deposit` diganti dengan `choose-goal-funding`/`goal-fund` sehingga dismiss guard patch tetap terbawa pada flow baru.
...
- `Bayar kewajiban`, Target, dan Investasi melewati decision point minimum: satu pilihan valid diteruskan otomatis; banyak pilihan meminta user memilih. Target membuka pendanaan canonical `cash/investment/mixed`, sedangkan Investasi membuka Buy/setup dengan konteks yang diketahui tanpa mutation duplikat di launcher.
- Flow Target quick-record tidak mengisi nominal secara sewenang-wenang. Rekening, portfolio/aset, dan nominal hanya diprefill bila konteks eksplisit atau hanya ada satu pilihan valid; picker tetap memakai komponen canonical project.
- Sumber pemasukan memakai ranking histori rekening tujuan yang sama sebagai **urutan rekomendasi**, bukan auto-selection, dan copy picker `Sumber` memakai placeholder/search/group yang sesuai domain.
- Pengeluaran/Pemasukan/Transfer membuka composer canonical dengan jenis terkunci, nominal-first, tanggal visible, dan satu CTA footer; user dapat kembali ke launcher tanpa memilih jenis transaksi ulang. Refund tetap capability transaksi tetapi bukan quick action global.
- Flow Pengeluaran generic memakai decision 0/1/banyak kandidat pada **Penggunaan dana**; nol kandidat memakai **Dana Tersedia**, satu kandidat dapat auto-link namun tetap terlihat/editable, dan banyak kandidat meminta pilihan user. Overspend/duplicate/period-lock/approval guard backend tetap canonical.
...
- Detail Alokasi menampilkan nominal **sisa aktual** (`rencana - terpakai`) sebagai angka utama; nominal awal tetap menjadi konteks dan disederhanakan pada layar sempit agar angka sisa tidak terpotong.
- Quick action `+` dari sebuah Kebutuhan sekarang mengunci konteks Kebutuhan + Alokasi pada composer transaksi dan menampilkan status `Dari Alokasi`, sehingga user tidak perlu memilih ulang relasi yang sudah diketahui.
- Kontrak quick action Kebutuhan sekarang memakai **planning intent eksplisit** yang terpisah dari draft biasa; callback `Catat pengeluaran` level Alokasi dan tombol `+` per Kebutuhan juga dipisahkan agar context `budget_id` tidak dapat hilang karena jalur opsional/generic. `Tambah lagi` setelah transaksi contextual mempertahankan Kebutuhan yang sama, sedangkan aksi generic tanpa Kebutuhan tidak lagi diam-diam mengikat `envelope_period_id`. Planning intent kini sudah tersedia sejak render pertama, menonaktifkan smart-selection selama context terkunci, dan submit fail-closed bila ID context berubah; Kebutuhan `Sekali bayar` tidak menawarkan `Tambah lagi` setelah sukses, serta tanggal contextual dibatasi ke periode Alokasi.
- Pemilih **Alokasi manual** di composer transaksi desktop/mobile dihapus. Relasi Alokasi mengikuti Kebutuhan yang dipilih atau smart matching canonical; pengeluaran biasa dapat memakai **Dana Tersedia** tanpa membuat Kebutuhan baru, sementara state belum memilih tetap hanya dipakai pada kondisi multi-kandidat yang ambigu.
- Aksi lifecycle di modal Edit Kebutuhan diubah dari label samar `Kelola status` menjadi `Hapus dari daftar`; server preview tetap menentukan apakah data aman dihapus permanen atau harus diarsipkan.
- Regression contract transaksi dan Honest Action Contract diperbarui mengikuti flow baru.
- Guard P0 transaksi membedakan **belum memilih Penggunaan dana** dari intent eksplisit **Dana Tersedia**: multi-kandidat wajib dipilih sebelum submit, perubahan rekening/kategori/tanggal menghitung ulang relasi, context Kebutuhan dari Alokasi dipertahankan, dan preview dampak menyebut sumber pengurangan dana secara eksplisit.
- Audit maintainability memakai hook `allocationWorkflowNavigation` canonical di `AllocationsWorkspace`, menurunkan ukuran workspace di bawah review threshold sekaligus menghapus named export tanpa consumer.


## Unreleased — Production migration recovery hardening

- Memperjelas flow **Kebutuhan → Catat transaksi**: daftar Kebutuhan kini menonjolkan **Sisa** yang benar-benar berubah mengikuti pemakaian, tombol `+` membawa Kebutuhan + Alokasi langsung ke composer walau snapshot overview belum tersinkron, picker **Alokasi manual** dihapus agar user tidak memilih dua kali, dan entry lifecycle lintas Rekening/Kategori/Alokasi/Kebutuhan/Jadwal/Target diganti dari `Kelola status` menjadi **Hapus dari daftar** sebelum preview server menentukan hapus permanen vs arsip.
- Memadatkan kembali detail **Alokasi Dana + Kebutuhan** untuk skenario daftar panjang: hero mobile memakai spacing/metric lebih ringkas, grouped Kebutuhan menjadi dense inline rows, aksi Catat/Jadwal berpindah ke quick action icon-only 44px, Edit tetap di overflow, status perhatian menggantikan chip pola, dan filter ringan `Semua / Perhatian / Belum dipakai` muncul otomatis saat item >4. Regression + QA/design contract ikut diselaraskan tanpa mengubah API, schema, authorization, ledger, atau perhitungan finansial.
- Merapikan detail **Kebutuhan di dalam Alokasi Dana** menjadi compact financial list: nominal Sisa dan progress lebih mudah dipindai, pola penggunaan dipadatkan menjadi chip, badge `Aman` disembunyikan pada kondisi normal, status perhatian tetap eksplisit, Edit dipindahkan ke overflow action 44px, dan Tambah kebutuhan diletakkan setelah daftar. Perubahan hanya presentation/UX; contract finansial, API, schema, dan authorization tidak berubah.
- Menambahkan **Honest Action Contract** project-wide: lifecycle Alokasi/Kebutuhan/Rekening/Kategori/Jadwal/Target membedakan `Hapus permanen` dari `Arsipkan` setelah preview server, Kewajiban memakai `Hentikan kewajiban` untuk `commitments.archive`, Target memakai `Setor dana`/`Tarik dana`, aktivasi Push mengungkap notifikasi uji otomatis, login menjelaskan model akses Administrator, dan user-facing Investasi/Planning dibersihkan dari istilah legacy yang tidak lagi menjadi hierarchy utama. Regression + policy docs diperluas agar label, konsekuensi mutation, feedback, recovery, serta otomatisasi finansial tetap jujur dan konsisten.
- Menyelesaikan **project-wide UX simplification pass** pada Alokasi/Kebutuhan, Jadwal Rutin, Kewajiban, Transaksi/transfer, Target, Rekening, Rekonsiliasi, Investasi, Laporan, dan Dashboard: helper/notice/modal description yang redundan atau terlalu teknis dipangkas, pola Kebutuhan diseragamkan menjadi **Bisa dipakai beberapa kali / Sekali bayar / Rutin**, ringkasan Alokasi berfokus pada Perlu disiapkan/Tersedia/Sisa atau kekurangan, serta feedback finansial berbahasa hasil tanpa narasi server/backend/ledger/snapshot. Governance microcopy, QA, dan regression test diperluas agar normal state tetap tenang sementara warning finansial/destructive/recovery/error tetap eksplisit. Tidak ada perubahan schema, API contract, authorization, atau perhitungan finansial inti.
- Menambahkan UX simplification/action-ownership pass: Kewajiban dan Investasi kini hanya memiliki satu create CTA pada true-empty dan baru mengembalikan action header setelah data tersedia; Dashboard mobile tidak lagi menggandakan quick-add transaksi pada empty Aktivitas; quick-add global diberi identitas visual **Catat** dan seluruh create transaksi user-facing distandardkan menjadi **Catat transaksi**. Regression + QA contract diperluas agar duplicate CTA, filtered-empty create, dan ambiguity FAB mobile tidak kembali pada patch berikutnya.
- Memperbaiki false-negative rendered focus browser smoke lintas Windows/GitHub Actions: quality gate kini menavigasi fokus dengan event keyboard CDP `Tab` lalu memverifikasi `:focus-visible` dan outline aktual, bukan memakai `element.focus()` programatik. Regression tooling melarang pola lama agar silang Quality tidak berulang karena perbedaan input-modality browser.
- Menggabungkan patch Alokasi Dana candidate ke project utama dalam tree yang sama sebelum quality verification sehingga perubahan backend, UI, test, dan dokumentasi Alokasi tidak terlepas dari perbaikan GitHub Quality.
...
- Menyelaraskan backup/restore/reset, schema/data dictionary, API/authorization, product requirement, implementation matrix, deployment/test contract, dan regression ke schema v23. `notification_settings` ikut backup/restore; read-state presentation tidak masuk logical backup finansial dan dibersihkan maintenance.
- Menyatukan **Batch 3–4 Smart Planning & UX Lifecycle**: create Alokasi sekarang berbahasa intent `Atur uang` lalu langsung lanjut Kebutuhan; editor Kebutuhan menaruh Cara penggunaan pada progressive detail; Jadwal Rutin dan Komitmen dapat smart-link ke satu Kebutuhan kompatibel melalui `budget_id`, meminta pilihan saat ambigu, dan tetap mendukung pembayaran mandiri bila tidak ada kandidat. Link ini mencegah Dana Tersedia mencadangkan tagihan yang sudah didanai Alokasi untuk kedua kali.
- Menghapus checkbox Autodebet dari Komitmen dan memaksa seluruh write Komitmen/Jadwal managed baru `auto_debit=false` sambil tetap membaca histori legacy. Form Komitmen dibuat progressive, empty state planning diperjelas, dan payment occurrence tetap memakai service recurring canonical.
- Memperluas lifecycle dirty guard ke Komitmen create/edit/penerimaan Arisan, Bagi Dana manual, dan Pengingat manual; explicit Batal/Tutup tetap langsung menutup, sedangkan dismiss dirty meminta konfirmasi. Hook guard mendapat `markClean()` agar modal yang tetap terbuka setelah save tidak salah menganggap draft tersimpan sebagai perubahan belum disimpan.
- Menambahkan **Goal Achievement Postcard** setelah setoran Target dikonfirmasi server: ilustrasi tujuan menjadi fokus visual, nominal + progress tampil langsung, milestone 25/50/75/90% mendapat copy suportif, 100% mendapat celebration finite tanpa mengubah lifecycle Target otomatis menjadi `completed`, dan `goals.move` tidak lagi menggandakan GlobalProcessIndicator/toast. Reduced-motion mempertahankan informasi tanpa travel/confetti.
- Menambahkan **Komitmen** pada Perencanaan untuk KPR, cicilan, pinjaman, dan Arisan. Schema v22 menambah `commitments`/`commitment_movements`, menghubungkan satu Jadwal Rutin canonical per Komitmen, memisahkan pokok vs bunga/biaya bila sisa pokok diketahui, mendukung penerimaan Arisan, field autodebet legacy read-only dengan write baru selalu false, lifecycle selesai/reversal, report/export, notification, realtime invalidation, integrity, backup/restore, trial/full reset, dan dokumentasi governance.
- Menambahkan **trial notifikasi native di perangkat** pada `/pengaturan/notifikasi`: user dapat memilih tema Liburan atau Masa Depan Bersama lalu memicu `ServiceWorkerRegistration.showNotification()` langsung dari perangkat untuk mengecek bentuk compact/expanded OS, rich image best-effort, CTA/deep-link `/target`, dan izin PWA iPhone/Android sebelum copy/asset dipakai lebih luas. Dua ilustrasi transparan disimpan sebagai asset demo non-sensitif; flow ini lokal-only dan tidak mengubah queue/delivery Web Push server.
- Merapikan Dashboard menjadi **decision-first** tanpa berubah menjadi mini rekening-koran: **Dana Tersedia** tetap hero dengan helper “Sisa uang yang aman dipakai setelah kebutuhan dan tagihan.”; `Bulan ini` menjadi snapshot compact Masuk/Keluar/**Selisih**; shortcut depan menjadi **Atur Dana / Rekening / Target / Cocokkan**; `Perlu dilakukan` hanya muncul saat ada alert; Kebutuhan+Jadwal digabung menjadi **Rencana terdekat**; Aktivitas terbaru naik sebelum Investasi dan Investasi kosong tidak dirender. Standalone `Insight Keuangan` mobile dihapus karena hanya mengulang keadaan hero/alert.
- Mengubah hierarchy saldo Beranda: **Dana Tersedia** (`safeToSpend`) kini menjadi angka utama mobile/desktop, sedangkan Saldo rekening non-investasi turun menjadi konteks sekunder dan Aman dipakai / hari tetap terlihat. Read-model Dana Tersedia juga diperbaiki agar Jadwal Rutin yang sudah didanai melalui Kebutuhan dalam Alokasi tidak dicadangkan dua kali; ledger, saldo fisik, dan mutation finansial tetap tidak berubah. Alert dashboard kini diurutkan berdasarkan urgensi tindakan manusia, bukan severity+judul saja, tanpa menghidupkan kembali mismatch rekonsiliasi historis setelah checkpoint.
- Meningkatkan **Smart Transaction** tanpa schema/API baru: composer mobile memindahkan tanggal/metode/catatan ke `Detail tambahan`, satu Kebutuhan matching dihubungkan otomatis, kandidat ambigu wajib dipilih melalui Kebutuhan tanpa picker Alokasi manual terpisah, dan preview lokal menampilkan perkiraan sisa Kebutuhan/Dana Tersedia/saldo rekening. Preview edit sekarang mereverse dampak transaksi lama sebelum menerapkan nilai baru; setelah create, success overlay memakai snapshot server terbaru bila refresh berhasil. Flow terbaru menghapus konfirmasi dua langkah khusus unallocated, sementara duplicate/overspend guard serta transfer approval tetap dipertahankan. Sekaligus memecah complexity pada dialog/controller Kebutuhan tanpa menonaktifkan ESLint.
- Merapikan **Alokasi Dana → Tambah kebutuhan** menjadi editor compact: nama kebutuhan dan nominal sejajar, kategori memakai `InlineSelectionPicker` canonical yang sama seperti rekening/ATM di Transaksi, kategori baru dapat dibuat inline beserta ikon, dan pola kebutuhan dipisahkan menjadi `Fleksibel`, `Sekali bayar`, serta `Berulang`. Nama kebutuhan kini menjadi identitas rencana (contoh `Arisan PT`) sementara kategori tetap master klasifikasi (contoh `Arisan`). Schema v21 menambahkan `recording_mode`; transaksi memakai `budget_id` eksplisit untuk mencegah double-count saat beberapa kebutuhan berbagi kategori, dan `Sekali bayar` mem-prefill sisa nominal saat dicatat.
- Menambahkan Laporan per Alokasi end-to-end: `reports.monthly` menerima scope `allocation_rule_id` dan menghitung summary, tren, Kebutuhan, kategori, rekening, pencatat, serta transaksi dari scope backend yang sama; laporan global tetap tersedia. Halaman `/laporan` dirapikan menjadi responsive flat hierarchy yang hemat ruang di mobile tanpa mengubah shell/navigation aplikasi.
- Menambahkan unduhan laporan siap baca PDF dan Excel terolah dari report engine yang sama dengan UI. PDF memuat ringkasan, seluruh Alokasi/Kebutuhan/kategori dan transaksi secara multipage; Excel memakai sheet Ringkasan, Alokasi, Kebutuhan, Transaksi, Kategori, dan Rekening. Export lengkap Administrator yang sudah ada tetap kompatibel.
- Menambahkan patch quality loop wajib untuk agent/ChatGPT: setiap perubahan source harus menjalankan `npm run lint`, memperbaiki error sampai PASS, menjalankan targeted regression, lalu `npm run verify` pada tree final sebelum handoff. `npm run zip` tetap menjadi fail-closed archive gate terakhir, bukan tempat pertama kali menemukan lint regression.
- Memperbaiki regression refactor maintainability pada modul Kategori, Dashboard desktop, validasi transaksi, dan query Kebutuhan yang sebelumnya meninggalkan import/helper ownership tidak lengkap atau duplikat sehingga lint gagal.
- Refactor maintainability tanpa perubahan contract publik: memecah service Finance, Investasi, Kebutuhan, Reminder, scheduler Google/Push, Dashboard desktop, rekening mobile, dan dialog Kategori berdasarkan responsibility sambil mempertahankan facade/import consumer.
- Menambah governance `maintainability-boundaries`: runtime file >400 baris harus melalui review, exception memiliki growth ceiling, facade backend hasil decomposition wajib tetap tipis, dan shell frontend yang sudah dirapikan tidak boleh mengambil kembali responsibility yang diekstrak.
- Menyesuaikan source-regression/deletion policy agar mengikuti owner module baru tanpa melemahkan invariant transaksi, hard-delete allowlist, circular-dependency guard, atau behavior existing.
...
- Menghapus copy legacy `Cocokkan Investasi` dari form rekonsiliasi rekening umum; rekening Investasi tetap tidak ditawarkan oleh flow `Cocokkan Saldo`. Menambah regression contract dan QA rule agar initial render tanpa selection tidak dapat rusak kembali.
- Menyelaraskan `notifications.center` ke Authorization Matrix setelah full governance test menemukan contract read-only tersebut belum terdokumentasi, tanpa mengubah permission runtime.
...
- Menjadikan lazy interaction **shell-first** melalui `LazyActionFallback`: modal/sheet langsung mempunyai container, title, status aksesibel, dan skeleton field sementara chunk berat dimuat. Intent + idle prefetch dipusatkan di `actionModules.js`/`useActionPrefetch`, termasuk quick-add transaksi dan action kontekstual tanpa prefetch pada Save-Data/2G.
- Menambah continuity aplikasi saat dipakai seperti app: refresh silent setelah kembali dari background bila stale, reconnect refresh otomatis, status offline/recovering compact, `visualViewport` keyboard inset, app badge notifikasi bila didukung, serta PWA shortcut **Tambah transaksi** yang langsung membuka composer canonical. Tidak ada financial snapshot/session baru yang dipersist ke cache client.
- Memperkeras lifecycle modal/update: browser Back menutup modal lebih dulu, modal non-dismissible tetap tidak dapat dilewati saat mutation, service-worker update ditahan selama modal/composer/mutation aktif, dan transaction composer melindungi draft aktif dari reload. Service worker v11 tetap network-only untuk `/api`, tetapi precache app shell juga mengambil JS/CSS entry yang direferensikan HTML.
- Menyederhanakan manifest installed PWA dengan `standalone`/`minimal-ui` saja sampai custom window-controls overlay benar-benar diimplementasikan. Motion route internal yang masih satu mental workspace dibuat lebih subtle tanpa menambah exit animation, count-up nominal, atau motion dekoratif finansial baru. Tidak ada perubahan API, schema, ledger, authorization, idempotency, maupun business rule finansial.
...
- Memisahkan presentation **Saldo utama** dari Saldo RDN sampai read-model Laporan melalui `nonInvestmentBalance`, sehingga RDN tidak terbaca dua kali ketika user melihat saldo operasional dan investasi. `totalBalance` tetap tersedia sebagai field compatibility internal.
- Merapikan Alokasi Dana mobile: kartu overview menjadi ringkasan dengan satu pintu detail, aksi jarang dipakai dipusatkan di **Pengaturan alokasi**, tombol `Lainnya`/`Muat ulang` dan panel jadwal duplikat dihapus, serta **Dana yang disiapkan** menjadi opsional sampai kontrak backend.
- Menggabungkan pembuatan Kebutuhan dan Jadwal pembayaran dalam satu flow/modal. Jadwal tetap memakai action canonical `recurring.createRule` melalui application-level shared workflow agar boundary antar-feature tetap terjaga dan retry jadwal tidak menggandakan Kebutuhan.
- Menghapus surface Anggaran read-only yang menduplikasi Alokasi Dana. Navigasi aktif memakai Alokasi/Jadwal, sedangkan `/anggaran` dipertahankan sebagai compatibility redirect ke `/perencanaan/kantong`. Menu Notifikasi juga dihapus dari `Lainnya` mobile karena Notification Center sudah tersedia dari Dashboard.
- Membersihkan CSS/test presentasi yatim, menyelaraskan API contract, implementation matrix, project status, QA/test plan, design system, ADR, glossary, dan product requirements.
- Menutup quality gate ESLint tanpa menurunkan threshold atau memakai suppression: kompleksitas `AllocationsPage`, `BudgetDialogLayer`, `useBudgetActions`, validasi rekonsiliasi Investasi, dan pembuatan Alokasi backend dipecah menjadi helper kecil tanpa mengubah kontrak/behavior. Frontend regression 272/272 dan backend regression 513/513 lulus pada source final.
...
- Kartu/detail Rekening Investasi tetap merepresentasikan Cash RDN + ownership, sedangkan saham, lot/lembar, cost basis, harga manual/fallback, P/L, dan histori tetap authoritative di domain Investasi untuk mencegah dual ledger.

## 3 September 2026 - Rekening Investasi menjadi pintu ke saham aktual tercatat
- Menjadikan rekening `account_type=investment` sebagai pintu kontekstual ke Cash RDN dan saham yang benar-benar dicatat user: holding menampilkan lot/lembar, harga rata-rata, remaining cost basis, harga terakhir beserta sumber manual/trade fallback, nilai tercatat, realized/unrealized P/L, serta rincian aktivitas buy/sell/valuation/correction tanpa API broker atau market feed live.
- Menghubungkan `Tambah dana`/`Tarik dana` Investasi ke composer Transfer canonical dan menambahkan continuation aman setelah transfer RDN: transfer menuju RDN dapat lanjut ke Catat pembelian, tanpa auto-submit dan tanpa mengubah klasifikasi income/expense.
- Membuat kartu/detail Rekening Investasi membuka catatan saham untuk RDN yang sama; jika catatan belum ada, setup dibuka dengan RDN tersebut terpilih. RDN yang baru dibuat dari flow Investasi juga dibawa kembali sebagai context sehingga user tidak perlu memilih ulang rekening.
- Menambah modal rincian per saham yang lazy, memisahkan label harga manual vs fallback trade, menambahkan label Untung/Rugi selain warna, dan membuat `InvestmentOverview` lazy agar detail yang lebih kaya tidak mengembalikan route `/investasi` ke pelanggaran build-budget.
- Menutup temuan integritas baru pada event bertanggal sama: urutan Buy/Sell tidak lagi memakai UUID acak sebagai tie-breaker, dan snapshot valuation manual menang secara deterministik atas trade fallback bila tanggal/timestamp sama. Regression memaksa timestamp sama untuk menjaga weighted cost basis dan price-source tetap stabil.
- Memperbarui ADR/API/PRD/status/test plan agar field teknis `broker`/`name` hanya compatibility metadata; product contract tetap pencatatan manual aset aktual, bukan broker/trading client.
...
- Menolak visual palsu dari referensi: tidak ada Market Movers/harga live/performance time-series karena backend saat ini tidak mengekspos data tersebut; activity hanya Beli/Jual/Koreksi yang benar-benar ada pada read model.
- Menambah helper persentase P/L deterministik, regression presentation/UI, token semantic light/dark, reduced-motion, target sentuh mobile, dan form control 16px tanpa schema/API/dependency baru.
- Menyederhanakan setup Investasi menjadi fokus pada catatan: tidak ada pilihan jenis setup atau broker pada flow portfolio, hanya pilih RDN. Jika RDN belum ada, CTA membuka form Rekening dengan jenis Investasi terisi otomatis; rekening Investasi memakai nama internal otomatis dan kartu dibedakan sebagai Pribadi, Pasangan, atau Bersama.
- Menutup audit UX lanjutan: form Investasi kini memakai inline validation + focus error, seluruh dialog termasuk setup Portfolio/Instrumen mengunci payload `OUTCOME_UNKNOWN` sampai retry data yang sama, prerequisite Beli/Jual/Harga tampil eksplisit, dan setup RDN kosong mengarahkan ke Rekening tanpa dead-end.
- Memperbaiki contract Dashboard Investasi yang sebelumnya membaca `portfolio_count` yang tidak pernah dikirim backend; widget kini memakai keberadaan `portfolios` dan summary canonical yang sama pada desktop/mobile, sementara kegagalan resource Investasi tampil sebagai warning retryable tanpa memblokir dashboard utama.
- Memetakan feedback `investments.*`, `masterDataRequests.*`, `reminders.*`, dan `transferRequests.*`, menutup error handling review transfer/master data, serta mengunci keputusan+alasan ketika outcome review belum pasti sehingga hanya request yang sama dapat di-retry; global warning outcome write tetap tidak disupresi, tidak auto-dismiss, dan success/progress ganda dihindari.
- Melengkapi keyboard tab Planning/Pusat Persetujuan dengan roving focus Arrow/Home/End dan relasi tabpanel, serta memindahkan dialog Target ke lazy layer agar route Target kembali memiliki headroom build-budget.
...
- Menerapkan MVP RFC-0013 melalui schema v11 `009_transaction_cost_sharing.sql`: expense shared manual maupun occurrence Jadwal Rutin aktual mendukung pembagian beban `unspecified`, `equal`, atau `percentage` dengan snapshot integer Rupiah deterministik. Edit dengan mode yang sama mempertahankan participant/basis snapshot walau roster user berubah. Histori lama tetap unspecified, split tidak mengubah saldo ledger, audit/integrity/backup/restore/export ikut diperluas, dan laporan memisahkan pembagian beban biaya dari aktivitas pencatatan. Payer/beneficiary/kontribusi aktual serta refund-to-original-expense tetap deferred.
- Memperbaiki form Tambah rekening mobile menjadi jenis rekening 2 kolom dengan label utuh dan ownership ringkas. Tunai, Dana darurat, dan E-wallet memakai nama canonical/default saat nama manual tidak diperlukan tanpa mengubah contract backend `name` atau duplicate guard.
- Memecah `PlanningPage` dengan lazy boundary untuk Kantong dan Jadwal Rutin agar route tidak lagi membawa dua feature berat sekaligus; build budget tetap blocking dan threshold tidak dinaikkan.
...
- Tidak mengubah schema, saldo, transfer accounting, auth/role, backup/restore contract, API action, atau blocker canonical `periods.previewClose`; advisory yang tidak tersedia pada read-model selected-period tidak direka di frontend.

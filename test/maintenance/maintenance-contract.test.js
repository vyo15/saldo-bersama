...
test("backup schema v21 menyimpan data canonical termasuk investasi dan request kolaborasi tanpa session, binding runtime, atau bucket rate limit", async () => {
  ...
  try {
    ...
    assert.equal(snapshot.manifest.schemaVersion, 21);
...
test("backup schema v3-v20 tetap dapat dimuat ke schema v21 dengan field additive canonical", async () => {
  ...
  try {
    ...
    v16.tables.investment_trades = v16.tables.investment_trades.map(({ cash_effect_enabled: _cashEffect, ...row }) => row);
    v16.tables.investment_corrections = v16.tables.investment_corrections.map(({ cash_effect_enabled: _cashEffect, ...row }) => row);
    v16.checksum = digest(canonicalJson({ manifest: v16.manifest, tables: v16.tables }));
    assert.equal(validateSnapshot(v16), v16.checksum);

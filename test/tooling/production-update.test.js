...
test("migration target memakai schema_version SQL dan tidak menganggap prefix 017 sebagai v17", () => {
  ...
  assert.equal(migrationTargetSchemaVersion("UPDATE system_config SET value='22' WHERE key='schema_version';", "020_commitments.sql"), 22);
...
test("seluruh migration chain memisahkan migration ID dari target schema secara monoton", async () => {
  ...
  assert.deepEqual(migrations.map((item) => item.migrationId), Array.from({ length: 20 }, (_, index) => index + 1));
  assert.deepEqual(migrations.map((item) => item.targetSchemaVersion), Array.from({ length: 20 }, (_, index) => index + 3));
  assert.equal(migrations.at(-4)?.file, "017_budget_lifecycle_history.sql");
  assert.equal(migrations.at(-4)?.targetSchemaVersion, 19);
  assert.equal(migrations.at(-3)?.file, "018_envelope_decoration.sql");
  assert.equal(migrations.at(-3)?.targetSchemaVersion, 20);
  assert.equal(migrations.at(-2)?.file, "019_budget_recording_mode.sql");
  assert.equal(migrations.at(-2)?.targetSchemaVersion, 21);
  assert.equal(migrations.at(-1)?.file, "020_commitments.sql");
  assert.equal(migrations.at(-1)?.targetSchemaVersion, 22);
});
...
test("prod:update menjalankan staged update lalu menunggu runtime Production sehat", async () => {
  ...
  const result = await runProductionUpdate({
    ...
    runtimeCheck: async () => {
      calls.push(["runtime"]);
      return { health: { schema: { version: 22 } } };
    },
    logger: { log() {} },
  });
  assert.deepEqual(calls, [["remote", "update", "/tmp/project"], ["runtime"]]);
  assert.equal(result.promoted, true);
  assert.equal(result.runtime.health.schema.version, 22);
});
...
test("runtime verifier retry sampai alias baru terlihat", async () => {
  ...
  assert.equal(status.health.schema.version, 22);
});

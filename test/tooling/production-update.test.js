...
test("seluruh migration chain memisahkan migration ID dari target schema secara monoton", async () => {
  ...
  assert.deepEqual(migrations.map((item) => item.migrationId), Array.from({ length: 21 }, (_, index) => index + 1));
  assert.deepEqual(migrations.map((item) => item.targetSchemaVersion), Array.from({ length: 21 }, (_, index) => index + 3));
  assert.equal(migrations.at(-5)?.file, "017_budget_lifecycle_history.sql");
  assert.equal(migrations.at(-5)?.targetSchemaVersion, 19);
  assert.equal(migrations.at(-4)?.file, "018_envelope_decoration.sql");
  assert.equal(migrations.at(-4)?.targetSchemaVersion, 20);
  assert.equal(migrations.at(-3)?.file, "019_budget_recording_mode.sql");
  assert.equal(migrations.at(-3)?.targetSchemaVersion, 21);
  assert.equal(migrations.at(-2)?.file, "020_commitments.sql");
  assert.equal(migrations.at(-2)?.targetSchemaVersion, 22);
  assert.equal(migrations.at(-1)?.file, "021_notification_attention_state.sql");
  assert.equal(migrations.at(-1)?.targetSchemaVersion, 23);
});
...
test("prod:update menjalankan staged update lalu menunggu runtime Production sehat", async () => {
  ...
  const result = await runProductionUpdate({
    ...
    runtimeCheck: async () => {
      calls.push(["runtime"]);
      return { health: { schema: { version: 23 } } };
    },
    logger: { log() {} },
  });
  assert.deepEqual(calls, [["remote", "update", "/tmp/project"], ["runtime"]]);
  assert.equal(result.promoted, true);
  assert.equal(result.runtime.health.schema.version, 23);
});
...
test("runtime verifier retry sampai alias baru terlihat", async () => {
  ...
  assert.equal(status.health.schema.version, 23);
});

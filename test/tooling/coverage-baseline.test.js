...
test("schema helpers memverifikasi version canonical, environment binding, cache invalidation, dan checksum", async () => {
  ...
  try {
    ...
    invalidateSchemaCache();
    const readyDb = dbWith(DATABASE_SCHEMA_VERSION, "production");
    ...
    await assert.rejects(
      ...
      (error) => error?.code === "DATABASE_SCHEMA_MISMATCH"
        && error?.status === 503
        && !/npm run db:migrate/.test(error.message)
    ...
    process.env.VERCEL_ENV = "production";
    invalidateSchemaCache();
    const crossed = await readSchemaStatus(dbWith(DATABASE_SCHEMA_VERSION, "development"), { force: true });
    assert.equal(crossed.ready, false);
    assert.equal(crossed.environmentReady, false);
    await assert.rejects(
      () => assertDatabaseReady(dbWith(DATABASE_SCHEMA_VERSION, "development")),
    ...
    invalidateSchemaCache();
    const preview = await readSchemaStatus(dbWith(DATABASE_SCHEMA_VERSION, "production"), { force: true });
    ...
    invalidateSchemaCache();
    const local = await readSchemaStatus(dbWith(DATABASE_SCHEMA_VERSION, "unbound"), { force: true });
    ...
    assert.equal(missing.expectedVersion, DATABASE_SCHEMA_VERSION);

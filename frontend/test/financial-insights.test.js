...
test("laporan dan dashboard menampilkan insight lintas bulan serta peringatan actionable", async () => {
  ...
  assert.match(mobile, /dashboardUrgentAlerts\(overview\.alerts\)/);
...
test("laporan responsive memakai hierarchy compact, scope Alokasi, dan export terolah", async () => {
...
test("semua permukaan alert memakai kontrak guidance yang sama dan deep-link dikonsumsi satu kali", async () => {
  ...
  for (const label of ["Pastikan saldo sesuai", "Tambahkan dana alokasi", "Rapikan transaksi", "Periksa kebutuhan", "Periksa Alokasi Dana", "Catat pembayaran", "Buka tagihan ini", "Tambah dana"]) {
...
test("dashboard desktop dan mobile berbagi view model, sementara filter lengkap tetap canonical di desktop/transaksi", async () => {
  ...
  assert.match(setupChecklist, /detail: ready \? "Siap mencatat saldo dan transaksi" : "Tambahkan rekening yang akan dipakai"/);
  ...
  assert.doesNotMatch(desktop, /Masuk bulan ini/);
  assert.doesNotMatch(desktop, /Keluar bulan ini/);
  assert.equal((desktop.match(/>Catat<\/Button>/g) || []).length, 1);
  assert.match(mobile, /Saldo Keluarga/);
  assert.match(mobile, /Dana yang bisa kamu gunakan/);
  assert.match(mobile, /Sisa di Alokasi/);
  assert.doesNotMatch(mobile, /Selisih bulan ini/);
  assert.match(desktop, /Sembunyikan seluruh nominal/);
  assert.doesNotMatch(desktop, /overview\.alerts\.slice/);
  assert.match(mobile, /Aman dipakai[\s\S]{0,180}\/ hari/);
  assert.match(mobile, /overview\.familyBalance \?\? overview\.nonInvestmentBalance/);
...
test("continuity flow memakai prefill dan action existing tanpa mutation finansial otomatis", async () => {
  ...
  assert.match(reconciliation, /accountId: resultOverlay\.accountId/);

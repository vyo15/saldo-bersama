...
${accountEditors}`;
  ...
  assert.match(page, /useMediaQuery\(APP_MEDIA\.mobile\)/);
  ...
  assert.match(desktopWorkspace, /useMediaQuery\(APP_MEDIA\.desktop, \{ fallback: true \}\)/);
  ...
  assert.match(cardStyles, /\.visual\[data-has-image="false"\]::after\s*\{[^}]*background:\s*none;/s);
...
test("pencocokan saldo mobile memakai feedback lokal tanpa toast ganda dan celebration tetap aksesibel", async () => {
  ...
  assert.match(alertList, /styles\.mobileGuidance/);
  assert.doesNotMatch(alertList, /mobile-attention-guidance|mobile-attention-instruction/);

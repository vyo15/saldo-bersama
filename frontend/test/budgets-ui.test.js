...
test("form Kebutuhan memisahkan nama kebutuhan, kategori, pola pencatatan, dan pembuatan kategori berikon", async () => {
  ...
  assert.match(page, /createSharedCategory/);
  assert.match(page, /requestSharedCategoryCreation/);
...
test("detail Alokasi Dana menampilkan Kebutuhan dan Jadwal terkait tanpa membuat tab duplikat", async () => {
  ...
  assert.match(detail, /depleted \? patternLabel : status\.attention \? status\.label : patternLabel/);
  ...
  assert.doesNotMatch(detail, /Belum digunakan|Dana untuk kebutuhan ini sudah habis/, "Row compact tidak boleh mengulang status dengan helper panjang.");
  assert.match(detail, /allocation-limit-row__depleted-stamp[\s\S]*>Habis<\/span>/);
  assert.match(detail, /if \(depleted\) return[\s\S]*Tidak ada dana tersedia/);
  const depletedBranch = detail.match(/if \(depleted\) return([\s\S]*?)const usedPercent/)?.[1] || "";
  assert.match(depletedBranch, /Tidak ada dana tersedia/);
  assert.doesNotMatch(depletedBranch, /ProgressBar/);
...
test("kategori yang sama dapat dipakai pada beberapa Alokasi Dana tanpa menduplikasi master kategori", async () => {
  ...
  assert.match(controller, /String\(item\.name \|\| ""\)/);
  assert.match(controller, /findBudgetForForm/);
  assert.match(controller, /item\.budget_id === form\.budget_id/);
...
test("Tambah Kebutuhan pada detail Alokasi memakai batch compact tanpa menggandakan flow edit", async () => {
  ...
  assert.match(batchEditor, /recurring/);

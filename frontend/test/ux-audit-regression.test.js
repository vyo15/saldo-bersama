...
test("Investasi mengunci intent ketika outcome write belum pasti dan hanya mengizinkan retry payload yang sama", async () => {
  ...
  assert.match(dialog, /fieldset className=\{formStyles\.intentFieldset\} disabled=\{state\.outcomeUnknown\}/);
  assert.match(dialog, /dismissible=\{!state\.busy && !state\.outcomeUnknown\}/);

...
const ActivityDialog = ({ panelRef, closeRef, member, onClose, bodyProps }) => <div className={styles.memberActivityBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={panelRef} className={styles.memberActivityPanel} role="dialog" aria-modal="true" aria-labelledby="member-activity-title" tabIndex={-1}><header className={styles.memberActivityHeader}><div className={styles.memberActivityHeading}><strong id="member-activity-title">Aktivitas member</strong><small>Transaksi berdasarkan pencatat yang diverifikasi backend.</small></div><button ref={closeRef} className={styles.memberActivityClose} type="button" onClick={onClose} aria-label="Tutup aktivitas member"><FiX className={styles.memberActivityDesktopIcon} aria-hidden="true" /><FiArrowLeft className={styles.memberActivityMobileIcon} aria-hidden="true" /></button></header><ActivityBody member={member} {...bodyProps} /></section></div>;

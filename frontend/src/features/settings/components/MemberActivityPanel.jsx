...
const ActivityDialog = ({ panelRef, closeRef, member, onClose, bodyProps }) => <div className={activityStyles.memberActivityBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={panelRef} className={activityStyles.memberActivityPanel} role="dialog" aria-modal="true" aria-labelledby="member-activity-title" tabIndex={-1}><header className={activityStyles.memberActivityHeader}><div className={activityStyles.memberActivityHeading}><strong id="member-activity-title">Aktivitas anggota</strong><small>Transaksi berdasarkan pencatat yang diverifikasi backend.</small></div><button ref={closeRef} className={activityStyles.memberActivityClose} type="button" onClick={onClose} aria-label="Tutup aktivitas anggota"><FiX className={activityStyles.memberActivityDesktopIcon} aria-hidden="true" /><FiArrowLeft className={activityStyles.memberActivityMobileIcon} aria-hidden="true" /></button></header><ActivityBody member={member} {...bodyProps} /></section></div>;

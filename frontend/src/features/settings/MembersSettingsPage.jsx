...
const MemberStatusBadges = ({ member }) => <div className={styles.memberMeta}><span className={`status-badge status-badge--${member.status === "active" ? "active" : "warning"}`}>{userStatusLabel(member.status)}</span>{member.status === "active" && member.identity_status === "pending" ? <span className="status-badge status-badge--warning">Menunggu login</span> : null}{member.is_current ? <span className="status-badge">Akun ini</span> : null}</div>;
...
const MemberCard = ({ member, user, menuProps, setActivityMember }) => {
  ...
  const menuOpen = menuProps.openMenuId === member.user_id;
  return <Card className={styles.memberCard}><div className={styles.memberCardTop}><div className={styles.memberHeader}><UserAvatar user={member} className={styles.memberAvatar} /><span className={styles.memberCopy}><strong>{member.name || member.email}</strong><small>{member.email}</small><span className={styles.memberRole}>{roleLabel(member.role)}</span></span></div><MemberMenu member={member} menuOpen={menuOpen} {...menuProps} /></div><MemberStatusBadges member={member} /><Button className={styles.memberActivityButton} type="button" onClick={() => setActivityMember(member)}>Lihat aktivitas transaksi</Button></Card>;
};

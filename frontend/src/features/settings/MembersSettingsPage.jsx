...
const MemberToolbar = ({ searchQuery, setSearchQuery, roleFilter, setRoleFilter, count }) => <div className={memberStyles.memberToolbar}><label className={memberStyles.memberSearch}><FiSearch aria-hidden="true" /><span className="sr-only">Cari anggota</span><input type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Cari nama atau email" /></label><label className="field field--compact"><span className="sr-only">Filter role anggota</span><select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)} aria-label="Filter role anggota"><option value="all">Semua role</option><option value="owner">Administrator</option><option value="member">Member</option></select></label><span className={memberStyles.memberToolbarSummary}>{count} ditampilkan</span></div>;

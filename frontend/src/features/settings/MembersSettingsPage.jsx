...
const MembersContent = ({ resource, filteredMembers, toolbarProps, user, menuProps, setActivityMember }) => {
  if (resource.status === "loading") return <div className={styles.membersLoading} role="status">Memuat data anggota...</div>;
  if (resource.status === "error") return <ErrorState error={resource.error} onRetry={resource.reload} />;
  return <><MemberToolbar {...toolbarProps} count={filteredMembers.length} />{filteredMembers.length ? <div className={styles.memberGrid}>{filteredMembers.map((member) => <MemberCard key={member.user_id} member={member} user={user} menuProps={menuProps} setActivityMember={setActivityMember} />)}</div> : <EmptyState title="Anggota tidak ditemukan" description="Ubah kata pencarian atau filter role untuk menampilkan anggota lain." />}</>;
};

const MemberFormModal = ({ open, close, editingMember, memberForm, setMemberForm, saveMember, saving, result }) => <Modal open={open} onClose={close} title={editingMember ? "Ubah akses anggota" : "Tambah anggota"} description="Email dan role harus sama dengan ALLOWED_USERS_JSON. Backend tetap menjadi sumber keputusan akhir." size="sm" footer={<><Button type="button" onClick={close} disabled={saving}>Batal</Button><Button variant="primary" type="submit" form="member-access-form" loading={saving} disabled={saving}>Simpan akses</Button></>}><SettingsNotice result={result} /><form id="member-access-form" className="form-grid" onSubmit={saveMember}><label className="field form-grid__full"><span>Email Gmail *</span><input required type="email" disabled={Boolean(editingMember)} value={memberForm.email} onChange={(event) => setMemberForm((current) => ({ ...current, email: event.target.value }))} /><small>{editingMember ? "Email tidak dapat diganti saat edit. Tambahkan akun baru jika alamat Gmail berubah." : "Akun harus sudah tersedia pada allowlist deployment."}</small></label><label className="field form-grid__full"><span>Nama</span><input maxLength="120" value={memberForm.name} onChange={(event) => setMemberForm((current) => ({ ...current, name: event.target.value }))} /></label><label className="field form-grid__full"><span>Role</span><select value={memberForm.role} onChange={(event) => setMemberForm((current) => ({ ...current, role: event.target.value }))}><option value="member">Anggota</option><option value="owner">Pemilik</option></select><small>Backend menolak role yang berbeda dari allowlist.</small></label></form></Modal>;

const MemberActionModals = ({ target, actionState, setTarget, confirmUserAction }) => <><ConfirmationModal open={target?.action === "deactivate"} title="Nonaktifkan pengguna?" description={target ? `${target.member.email} tidak lagi dapat memakai aplikasi. Data keuangan dan audit tidak dihapus.` : ""} confirmLabel="Nonaktifkan pengguna" reasonLabel="Alasan penonaktifan" requireReason acknowledgementLabel="Saya sudah memastikan pengguna ini tidak memiliki data personal aktif yang perlu dipindahkan." busy={actionState.status === "submitting"} error={actionState.error} onCancel={() => actionState.status !== "submitting" && setTarget(null)} onConfirm={confirmUserAction} /><ConfirmationModal open={target?.action === "reactivate"} title="Aktifkan kembali pengguna?" description={target ? `${target.member.email} memperoleh akses kembali hanya jika email dan role cocok dengan ALLOWED_USERS_JSON.` : ""} confirmLabel="Aktifkan kembali" reasonLabel="Alasan reaktivasi" requireReason tone="primary" busy={actionState.status === "submitting"} error={actionState.error} onCancel={() => actionState.status !== "submitting" && setTarget(null)} onConfirm={confirmUserAction} /></>;

import { FiArrowRight, FiBell, FiMoreHorizontal, FiPieChart, FiPlus, FiRefreshCw, FiSliders } from "react-icons/fi";
...
import { allocationAssigneeLabel, allocationCardActionState, allocationSourceLabel, allocationUsage } from "./allocationPresentation.js";
...
const AllocationCard = ({ item, onOpenActions, onReminder, onAdjust, onAddNeed, canRemind, attention = false, onOpenDetail, needs = [], scheduleCount = 0 }) => {
  const usage = allocationUsage(item);
  const hasActions = Boolean(item.can_close || item.can_archive_rule);
  const actionState = allocationCardActionState(item, needs.length);
  ...
  return <Card className={allocationClass(`allocation-card${attention ? " allocation-card--attention" : ""}`)} data-envelope-period-id={item.envelope_period_id}>
    <div className={allocationClass("allocation-card__header")}><span className={allocationClass("allocation-card__icon")}><FiPieChart aria-hidden="true" /></span><div className={allocationClass("allocation-card__heading")}><h2>{item.name}</h2><p>{allocationAssigneeLabel(item)} · {allocationSourceLabel(item)}</p></div><div className={allocationClass("allocation-card__header-actions")}>{canRemind ? <button type="button" className={allocationClass("allocation-card__menu")} aria-label={`Atur pengingat alokasi ${item.name}`} onClick={() => onReminder(item)}><FiBell aria-hidden="true" /></button> : null}{hasActions ? <button type="button" className={allocationClass("allocation-card__menu")} aria-label={`Kelola alokasi ${item.name}`} onClick={() => onOpenActions(item)}><FiMoreHorizontal aria-hidden="true" /></button> : null}</div></div>
    ...
    {actionState.showPlanningActions ? <div className={allocationClass("allocation-card__next-step")}>
      {actionState.needsEmpty && actionState.canManageNeeds ? <p><strong>Belum ada kebutuhan.</strong> Tambahkan kebutuhan agar dana ini punya rencana penggunaan yang jelas.</p> : null}
      <div className={allocationClass("allocation-card__planning-actions")}>
        {actionState.canManageNeeds ? <Button variant={actionState.addNeedVariant} icon={FiPlus} onClick={() => onAddNeed(item)}>Tambah kebutuhan</Button> : null}
        {actionState.canAdjust ? <Button icon={FiSliders} onClick={() => onAdjust(item, "fund")}>Atur dana</Button> : null}
      </div>
    </div> : !needs.length ? <p className={allocationClass("allocation-card__needs-preview")}>{needPreview}</p> : null}
...
const AllocationCards = ({ items, totalItems, onOpenActions, onReminder, onAdjust, onAddNeed, attentionEnvelopeId, budgets, recurringItems, onOpenDetail, canCreate, canRemindItem, linkedBudgetsForItem, relatedRecurringForItem, openCreate, clearFilter }) => <section className={allocationClass("allocation-grid")} aria-label="Daftar Alokasi Dana aktif">{items.length ? items.map((item) => {
  ...
  return <AllocationCard key={item.envelope_period_id} item={item} onOpenActions={onOpenActions} onReminder={onReminder} onAdjust={onAdjust} onAddNeed={onAddNeed} canRemind={canRemindItem(item)} attention={item.envelope_period_id === attentionEnvelopeId} onOpenDetail={onOpenDetail} needs={needs} scheduleCount={scheduleCount} />;
...
const AllocationOverviewLayer = ({
  activeItems, filteredActiveItems, allocationFilter, setAllocationFilter, setActionTarget, onReminder, onAdjust, onAddNeed,
  attentionEnvelopeId, budgets, recurringItems, onOpenDetail, canCreate, canMove,
  openCreate, openMove, reload, canRemindItem,
  linkedBudgetsForItem, relatedRecurringForItem,
}) => <>
  {activeItems.length ? <AllocationSummary items={activeItems} /> : null}
  {activeItems.length ? <div className={allocationClass(`allocation-header-actions allocation-header-actions--${canMove ? "with-move" : "simple"}`)}>
    {canCreate ? <Button className={allocationClass("allocation-header-actions__create")} icon={FiPlus} onClick={openCreate}>Alokasi baru</Button> : null}
  ...
  <section className={allocationClass("allocation-active")} aria-labelledby="allocation-active-title">
    ...
    <AllocationCards items={filteredActiveItems} totalItems={activeItems.length} onOpenActions={setActionTarget} onReminder={onReminder} onAdjust={onAdjust} onAddNeed={onAddNeed} attentionEnvelopeId={attentionEnvelopeId} budgets={budgets} recurringItems={recurringItems} onOpenDetail={onOpenDetail} canCreate={canCreate} canRemindItem={canRemindItem} linkedBudgetsForItem={linkedBudgetsForItem} relatedRecurringForItem={relatedRecurringForItem} openCreate={openCreate} clearFilter={() => setAllocationFilter("all")} />
  </section>
</>;

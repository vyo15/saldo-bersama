...
const AllocationActionModal = ({ target, onClose, onClosePeriod, onLifecycle }) => <Modal open={Boolean(target)} onClose={onClose} title={target?.name || "Kelola Alokasi Dana"} description={target ? `${allocationAssigneeLabel(target)} · ${allocationPeriodLabel(target.period_start, target.period_end)}` : ""} size="sm" mobileSwipeToClose><div className="allocation-action-sheet"><div className="allocation-action-sheet__balance"><span>Sisa dana</span><Money value={target?.remaining_amount || 0} tone={Number(target?.remaining_amount || 0) < 0 ? "negative" : "default"} /></div><div className="allocation-action-sheet__actions">{target?.can_close ? <Button icon={FiArchive} onClick={() => onClosePeriod(target)}>Tutup periode</Button> : null}{target?.can_archive_rule ? <Button className="allocation-action-sheet__danger" icon={FiArchive} onClick={() => onLifecycle(target)}>Kelola data</Button> : null}</div><p>Aksi penutupan dan arsip tetap memakai validasi serta konfirmasi yang sama seperti sebelumnya.</p></div></Modal>;

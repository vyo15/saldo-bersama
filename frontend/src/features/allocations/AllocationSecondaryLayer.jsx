...
const AllocationActionModal = ({ target, onClose, onLifecycle }) => <Modal open={Boolean(target)} onClose={onClose} title={target?.name || "Kelola Alokasi Dana"} description={target ? `${allocationAssigneeLabel(target)} · ${allocationPeriodLabel(target.period_start, target.period_end)}` : ""} size="sm" mobileSwipeToClose><div className={allocationClass("allocation-action-sheet")}><div className={allocationClass("allocation-action-sheet__balance")}><span>Masih tersedia</span><Money value={target?.remaining_amount || 0} tone={Number(target?.remaining_amount || 0) < 0 ? "negative" : "default"} /></div><div className={allocationClass("allocation-action-sheet__actions")}>{target?.can_archive_rule ? <Button className={allocationClass("allocation-action-sheet__danger")} icon={FiTrash2} onClick={() => onLifecycle(target)}>Hapus Alokasi</Button> : null}</div><p>Transaksi dan riwayat yang sudah terjadi tetap dipertahankan agar saldo masa lalu tidak berubah.</p></div></Modal>;

...
const AllocationGoalExecutionModal = ({ goal, accounts, transferRoutes, initialSourceAccountId = "", suggestedAmount = 0, busy, onBusyChange, onClose, onSuccess }) => {
  ...
  const [sourceAccountId, setSourceAccountId] = useState(() => compatibleAccounts.some((account) => account.account_id === initialSourceAccountId) ? initialSourceAccountId : (compatibleAccounts[0]?.account_id || ""));
  ...
  return <Modal open={Boolean(goal)} onClose={() => !busy && onClose()} dismissible={!busy} title="Sisihkan ke Target" description={goal?.name || ""} footer={<><Button type="button" disabled={busy} onClick={onClose}>Batal</Button><Button type="submit" form="allocation-goal-execution" variant="primary" loading={busy}>Sisihkan</Button></>}>

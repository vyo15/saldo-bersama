...
const initialFundingForm = ({ accounts, items, requestedAccountId, requestedEnvelopePeriodId, suggestedAmount }) => {
  ...
  return { sourceAccountId, envelopePeriodId: requestedEnvelope || (envelopes.length === 1 ? envelopes[0].envelope_period_id : ""), amount: suggestedAmount > 0 ? String(suggestedAmount) : "", reason: suggestedAmount > 0 ? "Menyesuaikan dana dengan total Kebutuhan" : "" };
};
...
const FundingFields = ({ accounts, envelopes, selectedAccount, form, setForm, changeSource, available, invalidAmount, amountNumber }) => {
  ...
  return <>
    <InlineSelectionPicker className="form-grid__full" label="Dari rekening" required value={form.sourceAccountId} onChange={changeSource} placeholder="Pilih rekening" placeholderOption={{ icon: AccountIcon }} searchable={accounts.length > 8} searchPlaceholder="Cari rekening…" options={accounts.map((account) => ({ value: account.account_id, label: accountDisplayLabel(account), meta: `Tersedia ${formatRupiah(availableBalance(account))}`, ...accountOptionVisual(account) }))} />
    <InlineSelectionPicker className="form-grid__full" label="Ke Alokasi Dana" required value={form.envelopePeriodId} onChange={(envelopePeriodId) => setForm((current) => ({ ...current, envelopePeriodId }))} placeholder="Pilih Alokasi Dana" placeholderOption={allocationOptionVisual()} searchable={envelopes.length > 8} searchPlaceholder="Cari Alokasi Dana…" options={envelopes.map((item) => ({ value: item.envelope_period_id, label: item.name, meta: `Sisa ${formatRupiah(item.remaining_amount || 0)}`, ...allocationOptionVisual() }))} />
    ...
    {selectedAccount && invalidAmount && amountNumber > available ? <div className="notice notice--warning form-grid__full" role="status">Dana kurang {formatRupiah(amountNumber - available)}.</div> : null}
...
const AllocationFundingFlow = ({ open, accounts, items, initialSourceAccountId = "", initialEnvelopePeriodId = "", suggestedAmount = 0, busy = false, error = null, onClose, onSubmit }) => {
  ...
  return <Modal open={open} onClose={guard.requestClose} discardGuard={guard} discardSubject="pembagian dana" dismissible={!busy} title="Bagi dana tersedia" description="Pembagian tidak mengubah saldo rekening." size="sm" footer={<><Button type="button" disabled={busy} onClick={guard.discardAndClose}>Batal</Button><Button type="submit" form="allocation-funding-form" variant="primary" loading={busy} disabled={!target || invalidAmount || busy}>Tambahkan</Button></>}>

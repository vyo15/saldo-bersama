...
const emptyForm = () => ({ commitment_type: "mortgage", name: "", provider: "", original_amount: "", current_balance: "", installment_amount: "", total_installments: "", installments_paid: "0", default_account_id: "", category_id: "", frequency: "monthly", due_day: "10", payment_method: "transfer", auto_debit: false, start_date: todayInJakarta(), end_date: "", notes: "" });
const emptyReceipt = () => ({ item: null, amount: "", account_id: "", category_id: "", transaction_date: todayInJakarta(), payment_method: "transfer", note: "" });
const refreshKeys = ["commitments.list", "recurring.list", "transactions.list", "accounts.list", "reports.monthly", "app.initialState"];
...
const CommitmentCard = ({ item, onEdit, onArchive, onReceipt }) => {
  ...
  return <article className={styles.card}>
    ...
    <div className={styles.amountBlock}>
      <span>{arisan ? "Sisa setoran" : "Sisa kewajiban"}</span>
      <strong>{formatRupiah(item.current_balance || 0)}</strong>
      <small>{completed ? "Selesai" : `${progress}% selesai`}</small>
    </div>
...
const CommitmentForm = ({ form, setForm, accounts, categories, error, editing = false }) => {
  ...
  return <div className="form-grid">
    ...
    <label className="field form-grid__full"><span>Nama *</span><input required maxLength="100" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder={arisan ? "Contoh: Arisan Keluarga" : "Contoh: KPR Rumah"} /></label>
    <label className="field"><span>{arisan ? "Nama grup/penyelenggara" : "Bank/Penyedia"}</span><input maxLength="100" value={form.provider} onChange={(event) => setForm((current) => ({ ...current, provider: event.target.value }))} placeholder={arisan ? "Keluarga" : "BTN"} /></label>
    {!editing ? <MoneyInput id="commitment-original" label={arisan ? "Total putaran" : "Pinjaman/nilai awal"} value={form.original_amount} onChange={(original_amount) => setForm((current) => ({ ...current, original_amount }))} required={!arisan} /> : null}
    ...
    <MoneyInput id="commitment-installment" label={arisan ? "Setoran per periode" : "Cicilan per periode"} value={form.installment_amount} onChange={(installment_amount) => setForm((current) => ({ ...current, installment_amount }))} required />
    ...
    {!editing ? <label className="field"><span>Sudah dibayar</span><input type="number" min="0" value={form.installments_paid} onChange={(event) => setForm((current) => ({ ...current, installments_paid: event.target.value }))} /></label> : null}
    ...
    <label className="field"><span>Tanggal jatuh tempo *</span><input type="number" min="1" max="31" required value={form.due_day} onChange={(event) => setForm((current) => ({ ...current, due_day: event.target.value }))} /></label>
    <AccountPicker value={form.default_account_id} accounts={accounts} onChange={(default_account_id) => setForm((current) => ({ ...current, default_account_id }))} />
    <CategoryPicker value={form.category_id} categories={categories} onChange={(category_id) => setForm((current) => ({ ...current, category_id }))} />
    ...
    <label className={`${styles.switchRow} form-grid__full`}><input type="checkbox" checked={Boolean(form.auto_debit)} onChange={(event) => setForm((current) => ({ ...current, auto_debit: event.target.checked }))} /><span><strong>Autodebet</strong><small>Hanya penanda. Saldo tetap berubah setelah pembayaran aktual dikonfirmasi.</small></span></label>
...
const CommitmentsPage = () => {
  ...
  const reloadAll = async () => { invalidate(refreshKeys); await Promise.allSettled([resource.reload(), refreshOverview()]); };
  const submitCreate = (event) => { event.preventDefault(); return mutation.run(async () => {
    await createCommitment({ ...form, original_amount: form.original_amount ? assertPositiveRupiah(form.original_amount) : undefined, current_balance: form.current_balance === "" ? undefined : Number(form.current_balance), installment_amount: assertPositiveRupiah(form.installment_amount), total_installments: Number(form.total_installments || 0), installments_paid: Number(form.installments_paid || 0), due_day: Number(form.due_day) });
    setCreateOpen(false); setForm(emptyForm()); notify({ message: "Komitmen dan Jadwal Rutin berhasil dibuat.", tone: "success", dedupeKey: "commitments:create" }); await reloadAll();
  }).catch(() => undefined); };
  const openEdit = (item) => { mutation.reset(); setEdit({ ...item, installment_amount: String(item.installment_amount || ""), total_installments: String(item.total_installments || ""), due_day: String(item.due_day || 1) }); };
  const submitEdit = (event) => { event.preventDefault(); if (!edit) return; return mutation.run(async () => {
    await updateCommitment({ commitment_id: edit.commitment_id, row_version: edit.row_version, name: edit.name, provider: edit.provider, installment_amount: assertPositiveRupiah(edit.installment_amount), total_installments: Number(edit.total_installments || 0), default_account_id: edit.default_account_id, category_id: edit.category_id, frequency: edit.frequency, due_day: Number(edit.due_day), payment_method: edit.payment_method || "transfer", auto_debit: Boolean(edit.auto_debit), start_date: edit.start_date, end_date: edit.end_date || "", notes: edit.notes || "" }, { rowVersion: edit.row_version });
    setEdit(null); notify({ message: "Komitmen dan jadwal berikutnya berhasil diperbarui.", tone: "success", dedupeKey: "commitments:update" }); await reloadAll();
  }).catch(() => undefined); };
  const submitReceipt = (event) => { event.preventDefault(); if (!receipt.item) return; return mutation.run(async () => {
    ...
    setReceipt(emptyReceipt()); notify({ message: "Penerimaan Arisan dan saldo rekening berhasil dicatat.", tone: "success", dedupeKey: "commitments:receipt" }); await reloadAll();
  }).catch(() => undefined); };
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.header}><div><h2>Komitmen</h2><p>Pantau KPR, cicilan, pinjaman, dan Arisan tanpa mencampurnya dengan Target.</p></div><Button variant="primary" icon={FiPlus} onClick={() => { mutation.reset(); setForm(emptyForm()); setCreateOpen(true); }}>Tambah Komitmen</Button></div>
    {items.length ? <><CommitmentSummary items={items} /><section className={styles.grid}>{items.map((item) => <CommitmentCard key={item.commitment_id} item={item} onEdit={openEdit} onArchive={(target) => { setArchiveError(null); setArchiveTarget(target); }} onReceipt={(target) => { mutation.reset(); setReceipt({ ...emptyReceipt(), item: target, amount: String(Math.max(0, Number(target.original_amount || 0) - Number(target.received_amount || 0)) || ""), account_id: target.default_account_id || "" }); }} />)}</section></> : <EmptyState icon={FiHome} title="Belum ada Komitmen" description="Catat KPR, cicilan, pinjaman, atau Arisan. Jadwal pembayarannya akan muncul otomatis di Jadwal Rutin." action={<Button variant="primary" icon={FiPlus} onClick={() => { mutation.reset(); setForm(emptyForm()); setCreateOpen(true); }}>Tambah Komitmen pertama</Button>} />}

    <Modal open={createOpen} onClose={() => !mutation.busy && setCreateOpen(false)} title="Tambah Komitmen" description="Satu kali dibuat, jadwal pembayaran akan terhubung otomatis ke Jadwal Rutin." footer={<><Button disabled={mutation.busy} onClick={() => setCreateOpen(false)}>Batal</Button><Button variant="primary" type="submit" form="commitment-create-form" loading={mutation.busy}>Simpan</Button></>}><form id="commitment-create-form" onSubmit={submitCreate}><CommitmentForm form={form} setForm={setForm} accounts={accounts} categories={expenseCategories} error={mutation.error} /></form></Modal>
    <Modal open={Boolean(edit)} onClose={() => !mutation.busy && setEdit(null)} title="Edit Komitmen" description="Perubahan jadwal diterapkan ke Jadwal Rutin berikutnya." footer={<><Button disabled={mutation.busy} onClick={() => setEdit(null)}>Batal</Button><Button variant="primary" type="submit" form="commitment-edit-form" loading={mutation.busy}>Simpan perubahan</Button></>}><form id="commitment-edit-form" onSubmit={submitEdit}>{edit ? <CommitmentForm form={edit} setForm={setEdit} accounts={accounts} categories={expenseCategories} error={mutation.error} editing /> : null}</form></Modal>
    <Modal open={Boolean(receipt.item)} onClose={() => !mutation.busy && setReceipt(emptyReceipt())} title="Catat penerimaan Arisan" description={receipt.item ? receipt.item.name : ""} footer={<><Button disabled={mutation.busy} onClick={() => setReceipt(emptyReceipt())}>Batal</Button><Button variant="primary" type="submit" form="commitment-receipt-form" loading={mutation.busy}>Catat penerimaan</Button></>}><form id="commitment-receipt-form" className="form-grid" onSubmit={submitReceipt}><MoneyInput id="commitment-receipt-amount" label="Nominal diterima" value={receipt.amount} onChange={(amount) => setReceipt((current) => ({ ...current, amount }))} required /><AccountPicker label="Masuk ke" value={receipt.account_id} accounts={accounts} onChange={(account_id) => setReceipt((current) => ({ ...current, account_id }))} /><CategoryPicker label="Kategori penerimaan" value={receipt.category_id} categories={incomeCategories} onChange={(category_id) => setReceipt((current) => ({ ...current, category_id }))} /><label className="field"><span>Tanggal diterima *</span><TemporalInput required type="date" value={receipt.transaction_date} onChange={(event) => setReceipt((current) => ({ ...current, transaction_date: event.target.value }))} /></label>{mutation.error ? <div className="notice notice--danger form-grid__full" role="alert">{mutation.error.message}</div> : null}</form></Modal>

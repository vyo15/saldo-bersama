...
const AccountOwnershipField = ({ entity, activeUsers, defaultOwnerUserId, currentOwnerLabel, onChange }) => {
  const currentUser = activeUsers.find((member) => member.is_current || member.user_id === defaultOwnerUserId)
    || (defaultOwnerUserId ? { user_id: defaultOwnerUserId, name: currentOwnerLabel, role: "member", is_current: true } : null);
  ...
  const options = [
    ...
    ...users.map((member) => ({
      ...
      label: "Saya",
      user: member,
      badge: userRoleLabel(member.role),
      badgeTone: member.role === "owner" ? "primary" : "neutral",
      description: "Rekening pribadi yang hanya dapat saya operasikan",
    })),
  ];
...
const BankTemplateField = ({ value, onChange, compact = false, showHelper = true }) => (
  <SelectionField className="form-grid__full" label="Template kartu bank" value={value} onChange={onChange} options={BANK_TEMPLATE_OPTIONS.map((item) => ({ ...item, ...bankTemplateOptionVisual(item.value) }))} helper={showHelper ? (compact ? "Template tersimpan sebagai tampilan kartu dan tidak mengubah nama rekening." : "Template hanya mengubah tampilan. PIN, CVV, nomor kartu debit, dan masa berlaku tidak disimpan.") : ""} />
);

const EwalletProviderField = ({ value, onChange, compact = false, showHelper = true }) => (
  <SelectionField className="form-grid__full" label="Provider E-wallet" value={value} onChange={onChange} options={EWALLET_PROVIDER_OPTIONS.map((item) => ({ ...item, ...ewalletTemplateOptionVisual(item.value) }))} helper={showHelper ? (compact ? "Provider tersimpan terpisah dari nama rekening dan hanya menentukan identitas visual E-wallet." : "Provider hanya menentukan identitas visual E-wallet.") : ""} />
);
    ...
    {accountTypeUsesAutomaticName(accountForm.account_type) ? <div className={`${styles.autoNameGroup} form-grid__full`}>
      ...
      <details className={styles.qualifierDisclosure} open={accountForm.account_type === ACCOUNT_TYPES.INVESTMENT ? true : undefined}>
        ...
        <label className="field">
          ...
          <small>{accountForm.account_type === ACCOUNT_TYPES.INVESTMENT ? "Contoh label: BCA ••••1234 · nama pemegang. Nama rekening tetap dibuat otomatis sebagai Investasi." : "Biarkan kosong bila satu rekening jenis ini sudah cukup."}</small>
        </label>
      </details>
  ...
  <>
    {editAccount?.account_type === ACCOUNT_TYPES.INVESTMENT
      ? <label className="field form-grid__full">
        ...
        <small>Nama rekening tetap Investasi; pembeda ini membantu saat Anda memiliki lebih dari satu RDN dengan kepemilikan yang sama.</small>
      </label>

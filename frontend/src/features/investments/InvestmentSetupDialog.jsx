...
const InvestmentSetupDialog = ({ accounts, owner, onClose, onSuccess }) => {
  const formRef = useRef(null);
  const [kind, setKind] = useState("portfolio");
  const [form, setForm] = useState({ name: "Ajaib", broker: "ajaib", lot_size: 100, exchange: "IDX" });
  ...
  const submit = async (event) => {
    ...
    try {
      await persistSetup(kind, createSetupPayload(kind, form));
      setOutcomeUnknown(false); invalidateInvestmentReads(); onSuccess(kind, form); onClose();
    } catch (caught) {

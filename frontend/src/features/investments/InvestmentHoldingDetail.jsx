...
const HoldingActivity = ({ portfolio, holding }) => {
  ...
  if (!items.length) return <p className={styles.inlineEmpty}>Belum ada aktivitas saham ini pada ringkasan histori terbaru.</p>;
  return <ul className={styles.activityList}>
    {items.map((item) => {
...
const InvestmentHoldingDetail = ({ portfolio, holding, onClose, onAction }) => {
  ...
  return <Modal open title={`Detail ${holding.ticker || "saham"}`} description="Detail holding aktual dari catatan investasi. Harga berasal dari catatan manual atau transaksi terakhir, bukan harga live." onClose={onClose} footer={footer}>
    <div className={styles.review}>
      ...
      <section className={styles.activitySection} aria-label={`Aktivitas ${holding.ticker || "saham"}`}>
        <div className={styles.sectionHeading}><div><h3>Aktivitas saham terbaru</h3><p>Riwayat terbaru yang memang terkait saham ini.</p></div></div>

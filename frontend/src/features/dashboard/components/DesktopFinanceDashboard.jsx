    ...
    <div className="dashboard-desktop premium-dashboard">
      <header className="premium-dashboard__header">
        ...
        <div className="premium-dashboard__actions">
          ...
          >
            ...
            <span>{balanceVisible ? "Sembunyikan nominal" : "Tampilkan nominal"}</span>
          </button>
      ...
      <section className="premium-metric-grid" aria-label="Ringkasan saldo">
        ...
        <article className="premium-metric-card">
          ...
          <SensitiveMoney visible={balanceVisible} value={overview.safeToSpend} />
          <small>Batas aman per hari <SensitiveMoney visible={balanceVisible} value={overview.dailySafeToSpend || 0} /></small>
          <div className="premium-mini-bars" aria-label="Perbandingan saldo rekening aktif">
            {accountBars.length ? accountBars.map((item) => (
              <span
                key={item.account_id}
                style={{ height: balanceVisible ? `${Math.max(12, Math.round((absoluteAmount(item.balance) / maxAccountBalance) * 62))}px` : "24px" }}
                title={balanceVisible ? `${item.name}: ${item.balance}` : `${item.name}: nominal disembunyikan`}
              />
        ...
        <article className="premium-metric-card">
          ...
          <div className="premium-category-chart" aria-label="Proporsi pengeluaran per kategori">
            {expenseBars.length ? expenseBars.map((item, index) => (
              <span
                key={`${item.name}-${index}`}
                style={{ height: balanceVisible ? `${Math.max(10, Math.round((absoluteAmount(item.amount) / maxCategoryExpense) * 58))}px` : "22px" }}
                title={balanceVisible ? `${item.name}: ${item.amount}` : `${item.name}: nominal disembunyikan`}
              />
        ...
        <article className="premium-metric-card premium-metric-card--accounts">
          ...
          <SensitiveMoney visible={balanceVisible} value={overview.unallocatedFunds || 0} />
          ...
          <div className="premium-account-strip" aria-label="Filter rekening cepat">
            {overview.accountBalances.slice(0, 3).map((item) => (
              <button key={item.account_id} type="button" className={accountFilter === item.account_id ? "active" : ""} onClick={() => setAccountFilter((current) => current === item.account_id ? "all" : item.account_id)} aria-pressed={accountFilter === item.account_id}>
                <strong>{item.name}</strong><SensitiveMoney visible={balanceVisible} value={item.balance} />
              </button>
            ))}
          </div>
        </article>
            ...
            {filteredTransactions.length ? filteredTransactions.map((item) => {
              ...
              return (
                <button type="button" className={`premium-transaction-row${active ? " active" : ""}`} key={item.transaction_id} onClick={() => setSelectedTransactionId(item.transaction_id)} aria-pressed={active}>
                  ...
                  <span className={`premium-transaction-amount money--${transactionTone(item.transaction_type)}`}>{sign}{sign ? " " : ""}<SensitiveMoney visible={balanceVisible} value={item.amount} tone={transactionTone(item.transaction_type)} /></span>
                </button>
              );
        ...
        <article className="premium-transaction-detail">
          {selectedTransaction ? (
            <>
              ...
              <div className="premium-detail-grid"><div><span>Nominal</span><SensitiveMoney visible={balanceVisible} value={selectedTransaction.amount} tone={transactionTone(selectedTransaction.transaction_type)} /><small>{TRANSACTION_LABELS[selectedTransaction.transaction_type] || selectedTransaction.transaction_type}</small></div><div><span>Rekening</span><strong>{transactionAccountLabel(selectedTransaction)}</strong><small>Sumber rekening tervalidasi</small></div><div><span>Alokasi</span><strong>{selectedEnvelope}</strong><small>{selectedEnvelopeNote}</small></div><div><span>Tanggal transaksi</span><strong>{formatTransactionDate(selectedTransaction.transaction_date)}</strong><small>Zona waktu Asia/Jakarta</small></div></div>

...
const MobileAttention = ({ alerts }) => {
  ...
  return (
    <section className={dashboardClass("mobile-finance-section mobile-reference-panel mobile-next-action-section")} aria-labelledby="mobile-next-action-title">
      <div className={dashboardClass("mobile-section-heading mobile-reference-panel__heading")}>
        <h2 id="mobile-next-action-title">Perlu Dilakukan</h2>
        <Link to="/notifikasi" state={{ returnTo: "/" }}>Lihat semua</Link>
      </div>
      <div className={dashboardClass("mobile-attention-list")}>
        {visibleAlerts.map((alert, index) => {
          ...
          return (
            <Link key={alert.id || alert.alert_id || `${alert.type || "alert"}-${index}`} className={dashboardClass("mobile-attention-row")} data-severity={alert.severity} to={guidance.to} state={guidance.state} aria-label={`${title}. ${fact}`}>
              <span className={dashboardClass("mobile-attention-row__icon")}><Icon aria-hidden="true" /></span>
              <span className={dashboardClass("mobile-attention-row__copy")}><strong>{title}</strong><small>{fact}</small></span>
              <span className={dashboardClass("mobile-attention-row__action")}>{guidance.actionLabel}</span>
              <FiChevronRight className={dashboardClass("mobile-attention-row__chevron")} aria-hidden="true" />
            </Link>
          );
        })}
      </div>
...
const MobileFinanceDashboard = ({ overview, viewModel, user, displayName, balanceVisible, onToggleBalance, onOpenQuickRecord, onOpenTransfer, onOpenTransactionDetail, setupContent }) => {
  ...
  return <section className={dashboardClass("mobile-finance-dashboard mobile-reference-dashboard")} aria-label="Ringkasan keuangan mobile">
    ...
    <div className={dashboardClass("mobile-finance-content mobile-reference-content")}>
      <section className={dashboardClass("mobile-quick-section")} aria-labelledby="mobile-quick-title">
        ...
        <DashboardQuickActions onOpenQuickRecord={onOpenQuickRecord} onOpenTransfer={onOpenTransfer} />
      </section>
      <MobileAttention alerts={urgentAlerts} />

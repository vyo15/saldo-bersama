...
const DashboardSurfaces = ({ mobileLayout, displayOverview, bootstrap, dashboardViewModel, investmentSummary, user, displayName, balanceVisible, setBalanceVisible, refreshOverview, isRefreshing, openTransactionComposer, openMobileTransactionDetail, desktopAccountId, setDesktopAccountId, filters, setters }) => {
  return (
    <Suspense fallback={<LoadingScreen variant="content" label="Menyiapkan ringkasan..." />}>
      {mobileLayout
        ? <MobileFinanceDashboard overview={displayOverview} viewModel={dashboardViewModel} investmentSummary={investmentSummary} user={user} displayName={displayName} balanceVisible={balanceVisible} onToggleBalance={() => setBalanceVisible((current) => !current)} onRefresh={refreshOverview} isRefreshing={isRefreshing} onOpenTransactionDetail={openMobileTransactionDetail} setupContent={<Suspense fallback={null}><FinancialSetupChecklist bootstrap={bootstrap} overview={displayOverview} user={user} /></Suspense>} />

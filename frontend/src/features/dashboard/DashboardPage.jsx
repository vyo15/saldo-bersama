...
const MobileDashboardOverlays = ({ mobileTransactionDetailOpen, setMobileTransactionDetailOpen, dashboardViewModel, balanceVisible, openTransactionComposer }) => <>{mobileTransactionDetailOpen ? (<Suspense fallback={<LazyActionFallback surface="modal" title="Detail transaksi" label="Menyiapkan detail transaksi..." />}><MobileTransactionDetail open onClose={() => setMobileTransactionDetailOpen(false)} transaction={dashboardViewModel.mobileSelectedTransaction} title={dashboardViewModel.mobileSelectedTitle} category={dashboardViewModel.mobileSelectedCategory} accountLabel={dashboardViewModel.transactionAccountLabel(dashboardViewModel.mobileSelectedTransaction)} envelope={dashboardViewModel.mobileSelectedEnvelope} envelopeNote={dashboardViewModel.mobileSelectedEnvelopeNote} lastSyncedAt={dashboardViewModel.lastSyncedAt} balanceVisible={balanceVisible} onOpenTransaction={() => { setMobileTransactionDetailOpen(false); openTransactionComposer(); }} /></Suspense>) : null}</>;

const DashboardSurfaces = ({ mobileLayout, displayOverview, bootstrap, dashboardViewModel, investmentSummary, user, displayName, balanceVisible, setBalanceVisible, openQuickRecord, openTransactionComposer, openMobileTransactionDetail, desktopAccountId, setDesktopAccountId, filters, setters }) => {
  return (
    <Suspense fallback={<NativePageSkeleton kind="dashboard" label="Menyiapkan ringkasan…" />}>
      {mobileLayout
        ? <MobileFinanceDashboard overview={displayOverview} viewModel={dashboardViewModel} investmentSummary={investmentSummary} user={user} displayName={displayName} balanceVisible={balanceVisible} onToggleBalance={() => setBalanceVisible((current) => !current)} onOpenQuickRecord={openQuickRecord} onOpenTransfer={() => openTransactionComposer({ initialType: "transfer" })} onOpenTransactionDetail={openMobileTransactionDetail} setupContent={<Suspense fallback={<div className="dashboard-deferred-card" aria-hidden="true" />}><FinancialSetupChecklist bootstrap={bootstrap} overview={displayOverview} user={user} /></Suspense>} />
        : <DesktopFinanceDashboard overview={displayOverview} bootstrap={bootstrap} viewModel={dashboardViewModel} investmentSummary={investmentSummary} displayName={displayName} selectedAccountId={desktopAccountId} onSelectAccount={(accountId) => { setDesktopAccountId(accountId); setters.setSelectedTransactionId(""); }} categoryFilter={filters.categoryFilter} setCategoryFilter={setters.setCategoryFilter} typeFilter={filters.typeFilter} setTypeFilter={setters.setTypeFilter} searchTerm={filters.searchTerm} setSearchTerm={setters.setSearchTerm} selectedTransactionId={filters.selectedTransactionId} setSelectedTransactionId={setters.setSelectedTransactionId} balanceVisible={balanceVisible} onToggleBalance={() => setBalanceVisible((current) => !current)} onOpenQuickRecord={openQuickRecord} onOpenTransfer={() => openTransactionComposer({ initialType: "transfer" })} setupContent={<Suspense fallback={<div className="dashboard-deferred-card" aria-hidden="true" />}><FinancialSetupChecklist bootstrap={bootstrap} overview={displayOverview} user={user} /></Suspense>} />}
    </Suspense>
  );
};

const DashboardPageContent = () => {
  const { overview, bootstrap, status, error, refreshAll } = useFinance(); const { openTransactionComposer } = useTransactionComposer(); const { openQuickRecord } = useQuickRecord(); const { user } = useAuth(); const mobileLayout = useMobileDashboardLayout();
  ...
  const surfaces = { mobileLayout, displayOverview, bootstrap: safeBootstrap, dashboardViewModel, investmentSummary, user, displayName, balanceVisible, setBalanceVisible, openQuickRecord, openTransactionComposer, openMobileTransactionDetail, desktopAccountId, setDesktopAccountId, filters, setters };

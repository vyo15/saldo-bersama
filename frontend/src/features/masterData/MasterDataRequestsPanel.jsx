...
const MasterDataRequestsPanel = ({ items = [], ownerMode = false, title, onApprove, onReject, busyId = "" }) => {
  ...
  return <section className={styles.panel} aria-labelledby="master-data-requests-title">
    ...
    <ConfirmationModal open={Boolean(rejectTarget)} title="Tolak pengajuan?" description={rejectTarget ? `${requestTitle(rejectTarget)} tidak akan menjadi data aktif.` : ""} confirmLabel="Tolak pengajuan" reasonLabel="Alasan penolakan" requireReason busy={busyId === rejectTarget?.request_id} onCancel={() => !busyId && setRejectTarget(null)} onConfirm={async (reason) => { if (!rejectTarget) return; const target = rejectTarget; await onReject(target, reason); setRejectTarget(null); }} />
  </section>;
};

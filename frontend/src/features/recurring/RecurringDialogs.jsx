...
const CategoryField = ({ value, categories, onChange }) => <InlineSelectionPicker label="Kategori" required value={value} onChange={onChange} placeholder="Pilih kategori" placeholderMeta="Pilih kategori jadwal" placeholderOption={categoryOptionVisual()} searchable={categories.length > 8} searchPlaceholder="Cari kategori…" options={categories.map((item) => ({ value: item.category_id, label: item.name, ...categoryOptionVisual(item) }))} />;

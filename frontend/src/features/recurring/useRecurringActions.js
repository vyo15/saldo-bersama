...
export const useRecurringRuleActions = (shared) => {
  ...
  const resetCreateDraft = () => { setForm(initialRuleForm()); setMessage(null); };
  const openCreate = () => { resetCreateDraft(); setCreateOpen(true); };
  const closeCreate = () => { if (!createMutation.busy) { setCreateOpen(false); resetCreateDraft(); } };
  const createRule = (event) => { event.preventDefault(); setMessage(null); return createMutation.run(async () => { await createRecurringRule({ ...form, budget_id: form.kind === "expense" ? form.budget_id || null : null, auto_debit: false, expected_amount: assertPositiveRupiah(form.expected_amount) }, {}); resetCreateDraft(); setCreateOpen(false); shared.notify({ message: "Jadwal rutin berhasil dibuat." }); await refreshRecurring(shared); }).catch((error) => setMessage({ type: "danger", text: error.message })); };
  const openRuleEditor = (item) => { setEditRule({ recurring_rule_id: item.recurring_rule_id, row_version: item.rule_row_version, name: item.name, kind: item.kind, expected_amount: String(item.rule_expected_amount || ""), frequency: item.frequency, due_day: Number(item.rule_due_day || 1), category_id: item.category_id, budget_id: item.budget_id || "", default_account_id: item.default_account_id, payment_method: item.payment_method === "autodebit" ? "transfer" : (item.payment_method || "transfer"), start_date: item.start_date || todayInJakarta(), end_date: item.end_date || "", priority: item.priority || "normal", status: item.rule_status || "active" }); setEditState({ status: "idle", error: null }); };
  const saveRule = async (event) => { event.preventDefault(); if (!editRule) return; setEditState({ status: "submitting", error: null }); try { await updateRecurringRule({ ...editRule, budget_id: editRule.kind === "expense" ? editRule.budget_id || null : null, auto_debit: false, expected_amount: assertPositiveRupiah(editRule.expected_amount) }, { rowVersion: editRule.row_version }); setEditRule(null); setEditState({ status: "idle", error: null }); shared.notify({ message: "Aturan rutin berhasil diperbarui." }); await refreshRecurring(shared); } catch (error) { setEditState({ status: "error", error }); } };
...
export const useRecurringPaymentActions = (shared) => {
  ...
  const reversePayment = async (reason) => { if (!reverseTarget?.reverse_transaction_id) return; const transactionId = reverseTarget.reverse_transaction_id; setReverseState({ status: "submitting", error: null }); try { await reverseRecurringPayment({ occurrence_id: reverseTarget.occurrence_id, transaction_id: transactionId, row_version: reverseTarget.row_version, reason }, { rowVersion: reverseTarget.row_version }); setReverseTarget(null); setReverseState({ status: "idle", error: null }); shared.notify({ message: "Pembayaran/penerimaan terakhir yang dapat Anda kelola berhasil dibatalkan dan status jadwal dihitung ulang." }); await refreshRecurring({ ...shared, keys: recurringLedgerRefreshKeys }); } catch (error) { setReverseState({ status: "error", error }); } };

...
const TransactionsPage = () => {
  const location = useLocation(); const { bootstrap, refreshOverview, invalidate } = useFinance(); const { openTransactionComposer } = useTransactionComposer(); const [draftQuery, setDraftQuery] = useState(""); const [filters, setFilters] = useState(() => initialFilters(location.state)); const [editingTransaction, setEditingTransaction] = useState(null); const resource = useApiResource("transactions.list", transactionQuery(filters)); const lifecycle = useTransactionLifecycle({ resource, refreshOverview, invalidate });

...
const EnvelopeField = ({ form, envelopes, update }) => <label className={`field ${styles.visualField}`} htmlFor="envelope"><span>Kantong/jatah</span><FieldControl icon={FiLayers}><select id="envelope" value={form.envelope_period_id} onChange={(event) => update("envelope_period_id", event.target.value)}><option value="">Belum dialokasikan</option>{envelopes.map((item) => <option key={item.envelope_period_id} value={item.envelope_period_id}>{item.name} — sisa {formatRupiah(item.remaining_amount)}</option>)}</select></FieldControl><small>Pengeluaran tanpa kantong masuk antrean review.</small></label>;
...
const TransactionFields = (p) => <>{p.lockType ? null : <TypeSelector form={p.form} update={p.update} />}{p.form.transaction_type === TRANSACTION_TYPES.INCOME ? <div className="notice notice--info form-grid__full"><span><strong>Contoh gajian:</strong> masukkan nominal, pilih rekening bank yang menerima gaji sebagai rekening tujuan, lalu pilih kategori Gaji. Saldo rekening akan bertambah setelah server mengonfirmasi transaksi.</span></div> : null}<AmountDateFields form={p.form} update={p.update} errors={p.errors} amountRef={p.amountRef} /><AccountCategoryFields {...p} /><div className="notice notice--info form-grid__full transaction-scope-note"><span>Ruang transaksi ditentukan otomatis dari kepemilikan rekening. Transfer lintas ruang ditolak agar saldo bersama tetap konsisten.</span></div><OptionalFields form={p.form} update={p.update} errors={p.errors} detailsOpen={p.detailsOpen} setDetailsOpen={p.setDetailsOpen} /><ImpactPreview impact={p.impact} isTransfer={p.isTransfer} />{p.confirmation ? <div className="notice notice--warning form-grid__full" role="alert"><FiAlertTriangle /><span>{p.confirmation.message} Periksa data, lalu tekan “Simpan tetap” untuk mengonfirmasi.</span></div> : null}{p.submitState.error ? <div className="notice notice--danger form-grid__full" role="alert">{p.submitState.error.message}</div> : null}</>;

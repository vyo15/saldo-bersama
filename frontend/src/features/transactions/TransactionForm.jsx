...
const TransactionForm = ({ open, onClose, initialType = TRANSACTION_TYPES.EXPENSE, transaction = null, onSaved }) => {
  const { bootstrap, overview, refresh } = useFinance();
  ...
  const handleSubmit = async (event) => {
    ...
    const prepared = {
      ...
      destination_account_id: isIncome ? form.destination_account_id : form.destination_account_id,
    ...
    try {
      ...
      const saved = await apiClient.request(action, validation.value, { idempotencyKey: idempotencyKeyRef.current, rowVersion: transaction?.row_version });
      await refresh();
      onSaved?.(saved);
    ...
    <Modal
      ...
      footer={(
        <>
          ...
          <Button type="submit" form="transaction-form" variant="primary" icon={submitting ? FiLoader : FiCheck} disabled={submitting}>
            {submitting ? "Menyimpan..." : confirmation ? "Simpan tetap" : transaction ? "Simpan perubahan" : "Simpan transaksi"}
          </Button>
        </>
      )}
    >
      <form id="transaction-form" className="form-grid" onSubmit={handleSubmit} noValidate>
        ...
        <div className="money-entry">
          <MoneyInput id="transaction-amount" value={form.amount} onChange={(value) => update("amount", value)} error={errors.amount} required />
        ...
        {impact ? (
          <div className="notice notice--info form-grid__full" aria-live="polite">

...
const EnvelopeField = ({ form, envelopes, update }) => <label className={`field ${styles.visualField}`} htmlFor="envelope"><span>Kantong (opsional)</span><FieldControl icon={FiLayers}><select id="envelope" value={form.envelope_period_id} onChange={(event) => update("envelope_period_id", event.target.value)}><option value="">Belum dialokasikan</option>{envelopes.map((item) => <option key={item.envelope_period_id} value={item.envelope_period_id}>{item.name} — sisa {formatRupiah(item.remaining_amount)}</option>)}</select></FieldControl></label>;

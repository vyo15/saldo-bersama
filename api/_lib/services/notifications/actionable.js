...
const queueEnvelopeNotifications = async (db, state, envelopes) => {
  ...
  for (const item of envelopes) {
    ...
    queued += await queueForRecipients(db, users, item, {
      ...
      body: `Terpakai + disiapkan ${notificationRupiah(Number(item.used_amount || 0) + Number(item.reserved_amount || 0))} dari ${notificationRupiah(item.allocated_amount)}. Sisa ${notificationRupiah(Math.max(0, allocated - Number(item.used_amount || 0) - Number(item.reserved_amount || 0)))}.`,
      targetPath: "/perencanaan/kantong",

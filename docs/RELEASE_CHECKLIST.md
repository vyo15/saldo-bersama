...
- [ ] Untuk schema v15, backup teknis **verified pada schema v14** tersedia sebelum migration Production; `013_investment_tracking.sql` diterapkan eksplisit setelah `012_member_collaboration.sql`, database di-bind ke environment yang benar, dan integrity lulus (`npm run db:integrity` untuk Development atau `npm run db:integrity -- production` untuk Production) **sebelum runtime schema baru dipush**. Production migration existing fail-closed bila tidak ada backup `verified` pada schema saat ini.
...
- [ ] Backup schema v15 memuat seluruh history Investment authoritative dan restore drill v14→v15 serta v15→v15 lulus foreign-key + business integrity; hasil restore dibandingkan untuk RDN, holding, cost basis, P/L, dan ledger parity sebelum success diklaim.

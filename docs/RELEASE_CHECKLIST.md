...
- [ ] Investment diuji end-to-end pada data test terisolasi: Bank→RDN tetap Transfer netral; **Saldo rekening/safe-to-spend/unallocated tidak memasukkan Cash RDN**; ordinary income/expense/refund/adjustment dengan RDN ditolak; Alokasi Dana/Jadwal Rutin baru tidak menerima RDN; Buy/Sell tidak menjadi income/expense; daily/monthly cash trend merekonsiliasi `investment_account_events`; total kekayaan saat ini tidak double-count Cash RDN; insufficient RDN/over-sell/stale `row_version`/IDOR ditolak; instrumen inactive tidak dapat dibeli tetapi holding existing dapat dijual; weighted cost basis + valuation/P&L benar; reconciliation as-of tidak auto-adjust; correction Administrator-only; tidak ada event sebelum `initial_balance_date` RDN.

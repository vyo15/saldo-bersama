...
- [ ] Untuk schema v17, backup teknis **verified pada schema v16** tersedia sebelum migration Production; `015_investment_asset_centric.sql` diterapkan setelah `014_investment_opening_position.sql`, database di-bind ke environment yang benar, dan integrity lulus (`npm run db:integrity` untuk Development atau `npm run db:integrity -- production` untuk Production) **sebelum runtime schema baru dipush**. Production migration existing fail-closed bila tidak ada backup `verified` pada schema saat ini.
...
- [ ] Backup schema v17 memuat seluruh history Investment authoritative + opening-position/trade notes + `is_system_hidden` + `cash_effect_enabled`; restore drill v16→v17 dan v17→v17 lulus foreign-key + business integrity. Hasil restore dibandingkan untuk hidden-account visibility, holding, cost basis, P/L, chronology, cash-effect flags, dan ledger parity sebelum success diklaim.

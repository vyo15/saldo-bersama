...
| Kewajiban | Implemented | KPR/cicilan/pinjaman/Arisan memakai ledger progres + satu Jadwal Rutin canonical. Utang flat dapat memisahkan pokok/bunga otomatis; Kewajiban yang Kebutuhan+Alokasinya funded penuh dicatat scheduler pada tanggal jatuh tempo, sedangkan yang belum funded tetap menunggu aktual. Penerimaan Arisan tetap income canonical. |
...
| Dashboard | Implemented | Dana Tersedia (`safeToSpend`) menjadi angka utama agar Saldo rekening tidak terbaca sebagai uang bebas; Saldo rekening non-investasi tetap konteks sekunder. Desktop memakai analytical workspace kaya data (hero cashflow aktual, attention, shortcut, analisis, rekening, ledger transaksi, lalu planning) tanpa mengubah curved sidebar canonical; mobile tetap decision-first untuk operasi harian. Shared view model, planning/attention state, investment overview, dan notification deep-link tetap canonical. |
| Laporan | Partial | Monthly/trend/breakdown tersedia termasuk aktivitas Komitmen (pokok, bunga/biaya, setoran/penerimaan Arisan); context bar periode/scope/unduh sticky di desktop dan tetap statis di mobile. Report document semester/tahunan masih future. |

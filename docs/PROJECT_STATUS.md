...
| Kebutuhan | Partial | Multi-item atomic maksimal 20. Menambah/mengubah Kebutuhan otomatis menyesuaikan dana Alokasi dari **Dana Tersedia**; saldo fisik baru berubah ketika transaksi terjadi. Dana kurang ditolak dengan `BUDGET_FUNDING_INSUFFICIENT` dan nominal gap. Hapus/archive hanya melepas sisa yang aman tanpa menyentuh dana terpakai/komitmen atau buffer sengaja. |
...
| Dashboard | Implemented | Shared view model, planning/attention state, investment overview, notification deep-link. |
...
- Kebutuhan adalah rencana nominal kategori di dalam Alokasi. Delta Kebutuhan secara otomatis fund/release Alokasi bila aman; operasi tersebut **tidak** membuat transaksi ledger.

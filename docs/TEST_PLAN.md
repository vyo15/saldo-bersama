...
## Hierarchical navigation & Back

- Root mobile (`/`, `/perencanaan/*` workspace root, `/transaksi`, dan root di Lainnya) tidak mendapat page-level Back universal.
- `/notifikasi` tersedia dari `Lainnya`, memakai contextual return yang tervalidasi saat dibuka dari menu tersebut, dan tetap fallback Beranda untuk direct/deep-link; `/rekonsiliasi` memakai contextual return + fallback Rekening; tidak ada `navigate(-1)` pada page biasa.
- `ContextBack` mempertahankan target sentuh minimal 44px dan label parent eksplisit.
- Detail Alokasi menyimpan `allocation` pada URL/history; browser/system Back harus menutup detail ke overview terlebih dahulu.
- Bottom-nav parent state diuji terpisah dari scroll restoration: Notifikasi boleh menyalakan Beranda tetapi harus mulai dari atas, sedangkan seluruh `/perencanaan/*` berbagi posisi tab Atur Dana.
- Overlay interaktif baru harus memakai `Modal` canonical; root memakai `×`, subview memakai `←`, dan hanya modal coordinator yang boleh memiliki `window.history.back()`/`popstate`.
- Regression canonical: `frontend/test/hierarchical-navigation.test.js`.
...
- Form create Kebutuhan wajib meminta **Cara penggunaan** secara eksplisit melalui inline picker canonical bergaya rekening/ATM; opsi **Sekali bayar / Bisa dipakai beberapa kali / Rutin** membawa ikon kecil dan tidak boleh memiliki default diam-diam. Flow create Alokasi + Kebutuhan memakai contract UI yang sama.
- Presentasi Kebutuhan membedakan state final secara eksplisit: `fixed_once` yang terpenuhi menampilkan **Selesai**, sedangkan `flexible/recurring` dengan sisa Rp0 menampilkan cap **Habis**. State Habis tidak merender progress bar atau primary `Catat/Bayar`; nominal terpakai dan fakta tidak ada dana tersedia tetap terlihat, sementara overflow/detail tetap dapat diakses.
- Edit Kebutuhan boleh mengubah Cara penggunaan hanya bila belum ada pemakaian (`used_amount=0`). Setelah terpakai selector menjadi read-only dengan alasan yang jelas dan backend wajib menolak perubahan; perubahan dari `recurring` sebelum pemakaian membersihkan/retire jadwal terkait, sedangkan perubahan ke `recurring` meminta jadwal baru.
- Picker kategori pada create Kebutuhan menyediakan aksi **Tambah kategori baru** inline; kategori owner yang baru dibuat otomatis dipilih kembali pada row/form aktif tanpa memaksa user pindah halaman.
...
- Surface **Atur Dana** menampilkan total compact Dana yang bisa dialokasikan lintas rekening + breakdown sumber tanpa menganggapnya satu pool uang. Dua CTA root wajib tetap tersedia: **Alokasikan dana** dan **Alokasi baru**. Root tidak memakai tiga tab domain; satu daftar **Aktif** menggabungkan Alokasi, standalone Jadwal Rutin expense, dan standalone Kewajiban. Recurring/Kewajiban yang sudah tertaut ke Kebutuhan/Alokasi aktif wajib didedup menjadi satu row Alokasi, sedangkan recurring income tidak boleh muncul pada Atur Dana. Flow generik **Alokasikan dana** wajib meminta rekening lebih dulu ketika ada beberapa sumber, lalu hanya menawarkan Alokasi dengan rekening sumber yang sama. Row Aktif wajib clickable sebagai satu surface, tidak memiliki CTA `Dana ke…` atau `Lihat detail`, dan hanya menampilkan signal kontekstual paling penting; detail/progres teknis ditunda ke halaman setelah klik. Root tidak membuat section `Perlu perhatian` terpisah; status masalah cukup inline pada item atau Notification Center. Adjustment contextual dari detail/attention mengunci rekening + tujuan. Impact preview wajib menunjukkan Dana bebas dan sisa Alokasi sebelum→sesudah serta Saldo rekening tidak berubah. Create user-facing memakai **Alokasi baru** dalam modal atomic dua langkah: tujuan + rekening/pengguna, lalu Kebutuhan awal. Rekening personal mengunci pengguna ke pemilik, rekening Bersama menawarkan Bersama/anggota sesuai capability. **Pemanis kartu** dan pilihan **Kembalikan ke dana tersedia / Tetap di alokasi berikutnya** tetap tersedia melalui **Tampilan & periode**; period type/start/end tidak diminta user. CTA canonical **Buat & alokasikan** atau **Buat dengan Rp…** saat dana hanya cukup sebagian.
...
- State koleksi wajib jujur: Kewajiban `completed` boleh menghasilkan copy **Semua kewajiban selesai**, sedangkan histori yang dihentikan/diarsipkan tidak boleh disamakan dengan selesai. Target completed-only tidak menampilkan ringkasan `0 target aktif`; kartu histori tetap dapat tampil tanpa hero progress palsu.
...
- Dashboard memakai **Saldo Keluarga** (`nonInvestmentBalance`) sebagai angka transparansi utama dan **Dana yang bisa kamu gunakan** (`safeToSpend`) sebagai konteks operasional actor. Breakdown **Saya / Pasangan / Bersama** harus konsisten dengan rekening operasional, uang personal pasangan tidak masuk `safeToSpend`, dan RDN/market value tidak boleh double-count. `safeToSpend` tetap memperhitungkan Alokasi Dana, proteksi, dan komitmen Jadwal Rutin operasional di luar Alokasi; Jadwal Rutin yang tertaut ke Kebutuhan yang sudah didanai Alokasi tidak boleh dikurangi dua kali.
- Hero mobile menguji **Saldo Keluarga** sebagai nominal utama, **Dana yang bisa kamu gunakan** sebagai konteks operasional, panel compact tiga kolom **Saya / Pasangan / Bersama**, serta **Aman dipakai / hari** dan **Sisa di Alokasi** sebagai metrik sekunder yang tetap terlihat. Semua nominal hero/breakdown ikut privacy toggle; breakdown memakai compact currency agar tiga kolom tidak overflow pada 320–430px.
- Mobile decision-first menguji urutan header freshness → hero Saldo Keluarga → ownership **Saya/Pasangan/Bersama** → ringkasan dana harian → `Perlu dilakukan` hanya untuk alert mendesak → quick action `Rekening / Target / Investasi` → Aktivitas terbaru. `Rencana terdekat`, ringkasan Atur Dana, dan card Investasi kedua tidak boleh kembali ke Beranda mobile karena sudah memiliki route canonical sendiri. `budget_threshold`/`envelope_threshold` tetap tersedia di Notification Center; standalone `Insight Keuangan`, standalone card `Bulan ini`, dan analitik bulanan tidak boleh kembali.
- Navigasi bawah mobile wajib berurutan **Beranda · Atur Dana · CATAT · Transaksi · Lainnya**; `Laporan` dan `Notifikasi` berada di menu sekunder `Lainnya` tanpa menambah bottom-nav item baru.
...
- Laporan tidak boleh merender `SummaryStrip`/KPI kedua setelah hero; global memverifikasi Selisih + fakta Masuk/Keluar/Saldo akhir, scope Alokasi memverifikasi Sisa + fakta Dialokasikan/Terpakai/Penggunaan, sedangkan Analisis lengkap tetap memuat Saldo awal periode/planning/Kewajiban/rekening/pencatat.
...
- Regression desktop wajib menjaga curved sidebar/header canonical milik `AppShell`, asset rekening canonical melalui `AccountVisual`, dan urutan Dashboard **saldo/attention → Rekening keluarga → shortcut Rekening/Target/Investasi → Aktivitas rekening + ringkasan Atur Dana/Investasi**. Analitik pengeluaran bulanan tidak boleh kembali ke Beranda karena menjadi tanggung jawab **analytical Report workspace**; freshness tetap hanya satu kali di header. Row-action overflow Transaksi, single page-level notice Target, management overflow Kewajiban, sticky report context desktop, contextual suppression floating quick-add, dan split workspace Rekonsiliasi >=1100px tetap diuji; behavior mobile canonical tidak boleh ikut berubah. Laporan tidak boleh menduplikasi business mutation atau mengganti shell/sidebar.
...
## UI density dan primitive consistency

- Kategori pada ≤36.25rem wajib menjadi satu kolom; tablet/desktop boleh mempertahankan multi-column bila copy dan target 44px tetap aman.

...
- Mobile decision-first menguji urutan hero Dana Tersedia compact → `Perlu dilakukan` hanya saat alert aktif → quick action `Atur Dana / Rekening / Target / Cocokkan` → `Rencana terdekat` gabungan Kebutuhan+Jadwal → Aktivitas terbaru → Investasi hanya bila ada nilai/holding; standalone `Insight Keuangan` dan standalone card `Bulan ini` tidak boleh kembali.

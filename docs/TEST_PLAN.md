...
- breakdown rekening/kategori/recorder hanya memakai transaksi aktif yang terlihat oleh actor; breakdown `nature` yang masih dikirim untuk compatibility legacy tidak boleh diangkat kembali sebagai UX aktif;
...
- `/laporan` mobile ≤820px menampilkan mode `Ringkasan` dan `Per kategori`, navigasi periode, pilihan **1 bulan harian** serta 3/6/12 bulan bulanan, chart pengeluaran, KPI arus kas/total saldo/saldo aman, kategori terbesar, alert actionable, anggaran vs aktual, serta rincian rekening/pencatat tanpa mutation; perbandingan bulan sebelumnya hanya berlaku untuk granularitas bulanan;
...
- Administrator melakukan keputusan pengajuan rekening, kategori, dan transfer melalui route owner-only `/persetujuan` yang mereuse action review existing; halaman domain Member tetap boleh memuat pengajuan miliknya, sedangkan owner tidak boleh memuat queue review duplikat di halaman Rekening/Kategori/Transaksi.
- `reports.monthly` dengan `trend_months=1` wajib menghasilkan satu titik per tanggal kalender untuk periode historis (termasuk 29 Februari) atau sampai hari Jakarta saat ini untuk bulan berjalan; transfer internal tidak boleh menambah income/expense/net dan tidak boleh mengubah total saldo lintas rekening hanya karena perpindahan internal.
- Expense tanpa `envelope_period_id` wajib berhenti sebelum mutation pertama, memberi warning eksplisit dan memerlukan konfirmasi user; validation error wajib mengarahkan fokus ke field invalid tanpa mengendurkan validasi backend.

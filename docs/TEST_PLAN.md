...
- Presentation `/investasi`: hero dan komposisi hanya memakai current-state `investments.overview`; persentase unrealized berasal deterministik dari cost basis positif; quick action mengikuti `can_operate`, Koreksi Administrator-only, activity hanya trade/correction yang benar-benar tersedia, dan UI tidak mengarang market discovery/performance time-series. Mobile `<=820px` menjaga target sentuh, native control 16px, dark/light token, nominal besar, serta reduced-motion; setup portfolio mengekspos Ajaib/Broker lain sesuai contract backend, tanpa RDN memberi next-step ke Rekening, dan Beli/Jual/Harga memberi prerequisite guidance. Form wajib menguji inline error + focus field pertama, sell melebihi holding, future date, serta `OUTCOME_UNKNOWN` yang mengunci payload dan hanya menawarkan retry data yang sama; guard yang sama berlaku pada setup Portfolio/Instrumen sehingga jenis setup, broker, RDN, ticker, dan nilai lain tidak dapat diubah sampai outcome definitif.

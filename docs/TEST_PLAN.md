...
- Tombol disabled selama auth/session diproses, mencegah double-submit, dan menampilkan error ramah tanpa raw Firebase error. Production menandai redirect intent, memulihkan `getRedirectResult` dengan fallback auth state hanya untuk intent yang valid, serta memakai watchdog agar kegagalan memulai redirect tidak terlihat freeze. `VITE_FIREBASE_AUTH_DOMAIN=saldo-bersama.firebaseapp.com` tetap upstream public config; `/__/auth/*` diproxy transparan pada Vercel dan OAuth redirect URI production wajib `https://saldo-bersama.vercel.app/__/auth/handler`.

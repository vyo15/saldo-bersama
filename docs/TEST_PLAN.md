...
- Mobile decision-first menguji urutan hero Dana Tersedia compact → `Perlu dilakukan` hanya saat alert aktif → quick action `Rekening / Target / Investasi` dan memastikan **Pastikan Saldo Sesuai** hanya muncul dari konteks Rekening → `Rencana terdekat` gabungan Kebutuhan+Jadwal → Aktivitas terbaru → Investasi hanya bila ada nilai/holding; standalone `Insight Keuangan` dan standalone card `Bulan ini` tidak boleh kembali.

...
- Mobile decision-first menguji urutan header freshness → hero Saldo Keluarga → ownership **Saya/Pasangan/Bersama** → ringkasan dana harian → `Perlu dilakukan` hanya untuk alert mendesak → quick action `Rekening / Target / Investasi` → `Rencana terdekat` gabungan Kebutuhan+Jadwal → Aktivitas terbaru → Investasi hanya bila ada nilai/holding. `budget_threshold`/`envelope_threshold` tidak boleh kembali sebagai card Dashboard dan tetap tersedia di Notification Center; standalone `Insight Keuangan` dan standalone card `Bulan ini` tidak boleh kembali.

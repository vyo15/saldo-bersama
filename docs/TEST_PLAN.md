...
- Mobile memakai satu tombol HTML branded Google milik Saldo Bersama dan Firebase Web SDK `GoogleAuthProvider` + `signInWithRedirect`; tidak memakai iframe `google.accounts.id.renderButton()`. Hasil redirect wajib dikonsumsi dengan `getRedirectResult` sebelum Firebase ID token diteruskan ke session backend existing.
- Tombol disabled selama redirect/session diproses, mencegah double-submit, menampilkan error ramah tanpa raw Firebase error, dan membersihkan auth client setelah token selesai dipakai. Production wajib memproxy `/__/auth/*` secara transparan ke helper Firebase dan memakai app domain sebagai `authDomain` runtime mobile.
- Network, unauthorized-domain, web-storage, dan provider error tetap kembali ke login dengan copy aman dan dapat dicoba ulang. Progress bar serta tombol back onboarding tidak tampil pada halaman login.
...
- Production manual QA wajib memastikan `saldo-bersama.vercel.app` ada di Firebase Authorized Domains, OAuth Web Client mengizinkan `https://saldo-bersama.vercel.app/__/auth/handler`, route `/__/auth/*` tidak jatuh ke SPA `index.html`, lalu menguji Android Chrome dan iPhone Safari/Home Screen bila tersedia.

...
- service worker tanpa API cache, tanpa cache Firebase auth helper `/__/auth/*`, hanya menyimpan response navigation HTML sebagai app shell, dan tanpa offline write queue;
...
- Mobile memakai satu tombol HTML branded Google milik Saldo Bersama dan tidak memakai `google.accounts.id.renderButton()`. Pada production canonical, Firebase Web SDK `GoogleAuthProvider` + `signInWithRedirect` memakai authDomain same-origin dan durable browser-local persistence; localhost/device emulation memakai `signInWithPopup` + in-memory persistence sebagai fallback development. Firebase ID token dari hasil auth diteruskan ke session backend existing agar backend session tetap menjadi authority.
- Tombol disabled selama auth/session diproses, mencegah double-submit, dan menampilkan error ramah tanpa raw Firebase error. Production menandai redirect intent, memulihkan `getRedirectResult` dengan fallback auth state hanya untuk intent yang valid, memakai watchdog agar kegagalan memulai redirect tidak terlihat freeze, dan menyiapkan Service Worker auth-safe sebelum tombol mobile aktif. `VITE_FIREBASE_AUTH_DOMAIN=saldo-bersama.firebaseapp.com` tetap upstream public config; `/__/auth/*` diproxy transparan pada Vercel, wajib bypass Service Worker/cache, dan OAuth redirect URI production wajib `https://saldo-bersama.vercel.app/__/auth/handler`.
...
- Production manual QA wajib memastikan `saldo-bersama.vercel.app` ada di Firebase Authorized Domains, Google provider aktif, OAuth Web Client memuat `https://saldo-bersama.vercel.app/__/auth/handler`, lalu menguji round-trip redirect pada Android Chrome dan iPhone Safari/Home Screen bila tersedia. Responsive emulation desktop tidak dianggap bukti real-device karena localhost tetap menggunakan popup fallback. Setelah deploy perubahan Service Worker, real-device QA juga memastikan worker lama tidak tetap menunggu di halaman login dan request `/__/auth/handler` tidak pernah dilayani dari app-shell cache.

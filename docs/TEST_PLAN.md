...
- Mobile memakai satu tombol HTML branded Google milik Saldo Bersama dan Firebase Web SDK `GoogleAuthProvider` + `signInWithRedirect`; tidak memakai iframe `google.accounts.id.renderButton()`. Hasil redirect wajib dikonsumsi dengan `getRedirectResult` sebelum Firebase ID token diteruskan ke session backend existing. Jika hasil redirect kosong tetapi redirect intent session masih valid, recovery boleh menunggu `Auth.authStateReady()` lalu memakai `currentUser`; tanpa intent valid tidak boleh auto-login dari state Firebase sisa.

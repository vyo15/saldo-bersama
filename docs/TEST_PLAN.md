...
- login desktop wajib memilih artwork approved light/dark berdasarkan theme, mempertahankan rasio 1672×941, dan hanya memakai satu host `.google-login-button` runtime tanpa mengganti flow Google Identity Services/Firebase; login mobile ≤820px wajib memiliki tepat empat halaman (tiga onboarding + login khusus), tiga onboarding memakai hero card clean dengan aset transparan terpisah, tanpa pill fitur di bawah deskripsi, tidak meniban area copy, serta muat satu layar tanpa scroll vertikal internal pada viewport mobile yang didukung. Halaman keempat mempertahankan login canonical dan money rain hanya saat aktif, progress bar serta tombol back onboarding tidak tampil, provider Google harus sepenuhnya berada di dalam slide tanpa clipping, memakai `size: medium` pada mobile dengan host responsif maksimal 300px dan label utuh. Host provider wajib transparan tanpa border/radius/shadow/clipping tambahan, placeholder/button inline harus hilang setelah iframe final siap, dan provider dipreload saat slide login masih off-screen. Browser smoke wajib membuktikan provider final siap sebelum halaman 4 bila jaringan/mock selesai lebih awal, perpindahan slide tidak merender ulang provider yang sudah siap, dan state `Menyiapkan login…` hanya dipakai bila halaman login dibuka sebelum iframe final selesai dimuat. “Selamat datang” tidak memakai garis eyebrow, kontrol `Lanjut`/`Lewati`/pagination minimum 44px, swipe/ArrowLeft/ArrowRight, creator link aman, theme toggle DOM asli, serta `prefers-reduced-motion` untuk transisi carousel;
...
- login mobile: Google Identity Services harus tetap menjadi kontrol canonical. Host halaman login dibuat responsif maksimal 300px dan provider meminta `size: medium` agar label utuh; host wajib transparan tanpa visual wrapper tambahan, iframe tidak di-resize paksa, placeholder inline dibuang setelah iframe final siap, dan seluruh kontrol provider harus berada di dalam viewport tanpa clipping. Provider dipreload ketika slide 4 masih off-screen, host disembunyikan sampai iframe final siap, perpindahan slide tidak boleh merender ulang provider, dan fallback `Menyiapkan login…` tampil hanya selama iframe provider final belum selesai dimuat; progress bar dan tombol back onboarding tidak tampil pada halaman login;

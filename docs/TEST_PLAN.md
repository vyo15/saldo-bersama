...
- Command isolation: `npm run dev` tidak boleh membuat/mengubah `.env.production.local`; `npm run prod` tidak boleh pull/menimpa `.env.local`. Bootstrap Development harus mencoba refresh Vercel, boleh memakai cache `.env.local` lengkap hanya ketika login/link/pull Vercel tidak tersedia, dan tetap fail-closed pada cache tidak lengkap, hasil pull invalid, Turso unreachable, schema, atau binding. Google bridge pusat hanya boleh diselaraskan ke profile Production lokal jika grup Development lengkap dan Production kosong; drift lengkap fail-closed.
...
- Create Alokasi wajib meminta `Nama alokasi`, rekening sumber, periode, dan dana yang disiapkan. Helper `Susun kebutuhan` memakai Kategori expense aktif, menolak kategori ganda/nominal nol/total melebihi dana tersedia, mengisi nominal saja, dan tidak memanggil `budgets.upsert` atau mutation finansial tambahan.
...
- Setup checklist menilai readiness pencatatan: minimal satu rekening operable serta kategori income+expense aktif. Alokasi Dana, Jadwal Rutin, dan Target tidak boleh menjadi blocker readiness. Guided continuation hanya muncul pada `setupFlow`; setelah kategori siap primary action menuju Transaksi, sedangkan planning tetap opsional.
...
- Dengan virtual keyboard terbuka, Search, nominal, textarea, date/month, `SelectionField`, dan sticky modal action tetap dapat dijangkau. `scrollWidth <= clientWidth + 1` berlaku pada page/dialog kecuali scroller horizontal yang memang intentional.
...
- manual device QA composer transaksi mobile pada 320/360/390/430px wajib memeriksa empat pilihan jenis transaksi tanpa wrapper ikon ganda, field nominal menyatu dan tidak overflow, grouped metadata tidak berubah menjadi card bertumpuk, rekening/Alokasi membuka selection view pada bottom sheet yang sama tanpa native dropdown/nested modal, sedangkan Kategori berkembang inline tanpa mengganti isi composer. Tombol kembali/Escape/swipe dari selection rekening/Alokasi harus mengembalikan composer tanpa kehilangan draft; rekening sumber tidak memiliki Search/Lihat semua dan rekening Rp0 tersaring sesuai jenis transaksi. Untuk Kategori, taxonomy >6 item wajib menampilkan `Cari kategori…`, shortcut `Sering dipakai` mengikuti histori rekening sumber, `Semua kategori` tidak menduplikasi shortcut, panel tetap bounded sekitar 15.5rem dengan internal scroll, search tetap usable ketika list panjang, pilihan menutup panel, dan draft/Alokasi suggestion tetap terjaga. Metode pembayaran compact tetap dapat dikosongkan, Auto-debit tidak tersedia untuk transaksi manual baru, Catatan auto-grow tanpa resize handle, dan `Setelah transaksi` tidak mengulang summary sebagai detail kedua;
- regression source scan wajib memastikan `frontend/src` tidak mengandung app-owned native `<select>`; pilihan dynamic memakai `SelectionField`/selection view canonical, sedangkan fixed option memakai `VisualChoiceGroup`/choice chips. `VisualChoiceGroup` wajib tetap menerima option tanpa icon, dan explanatory two-choice mode `descriptive` wajib menjaga radio semantics, equal-height tile, label wrap, selected check, description ringkas, focus-visible, serta reduced-motion. Manual QA harus memeriksa inline expansion mobile, popover desktop, Escape/outside dismiss, search list panjang, selected check, focus-visible, dan tidak ada clipping di modal scroll.
- Effective mobile CSS wajib memastikan trigger `SelectionField` default/compact/embedded, search field, notification filter/read-all, serta aksi link-style rekonsiliasi memiliki hit target minimum `--mobile-control-height` (44px). Visual boleh compact, tetapi area sentuh tidak boleh turun di bawah kontrak ini.
- login desktop wajib memakai komposisi floating-card canonical: hanya `couple-love.webp` sebagai asset visual desktop, artwork pasangan menjaga rasio intrinsik tanpa stretch vertikal dan tetap proporsional pada tinggi/lebar layout, tidak ada story copy tengah maupun badge fitur, floating auth card berdiri sendiri tanpa wrapper surface kedua, dan tidak ada divider/copy login email palsu. Card memuat logo project, welcome copy, tombol HTML branded Google, dan trust row Privat/Terverifikasi/Tersinkron; dark/light memakai token theme yang sama, bukan fallback poster terpisah. Login mobile ≤820px wajib memiliki tepat empat halaman (tiga onboarding + login khusus). Tiga onboarding memakai satu scene artwork transparan per slide (`Catat keuangan`, `Atur anggaran`, `Keuangan bersama`) tanpa hero card berbingkai, badge fitur, atau panel dekoratif tambahan; artwork tidak boleh meniban area copy dan setiap halaman harus muat satu layar tanpa scroll vertikal internal pada viewport mobile normal yang didukung. Swipe, pagination yang dapat diklik, `Lewati`, dan ArrowLeft/ArrowRight wajib bekerja; tombol `Kembali`/`Lanjut` sengaja tidak dirender agar onboarding tetap clean. Semua kontrol yang tersisa wajib memiliki target sentuh minimum 44px. Pada viewport sangat pendek atau kondisi text resize/zoom tinggi, page boleh scroll vertikal sebagai fallback agar CTA, creator link, dan navigation tetap reachable; clipping karena `height:100dvh` dilarang. Desktop dan halaman login mobile tidak merender tombol/iframe Google Identity Services. Production canonical wajib membuat tombol siap melalui transport server OAuth ringan tanpa mengimpor Firebase browser Auth; localhost/device emulation boleh lazy-load Firebase popup fallback. Setelah onboarding pernah mencapai slide login, refresh/kunjungan berikutnya pada perangkat yang sama langsung membuka login, kontrol `Ulang` tetap dapat membuka onboarding, dan storage hanya berisi flag presentasional. Slide tidak aktif tidak boleh memiliki Google button/retry focusable dan artwork onboarding tidak boleh dimuat sebelum slide aktif. Halaman login keempat tetap mempertahankan logo, money-rain, branded Google button, security hints, creator link, dan tidak menampilkan aksi `Kembali`/`Lanjut`. Manual device QA memeriksa tombol branded tunggal, target sentuh minimum 44px, desktop alignment, satu-layar pada viewport mobile relevan, swipe/keyboard, pagination tanpa back/next CTA, creator link, theme toggle, reduced motion, returning-user path, replay onboarding, dan round-trip auth pada perangkat nyata;
...
- Route Alokasi harus memuat `allocationActionRunners.js` secara on-demand saat mutation dijalankan; runner tersebut menjadi pemilik facade lokal `allocations.api.js` dan validator mutation. `AllocationsPage` tidak boleh mengimpor runner maupun facade mutation secara statis. Regression mengunci boundary ini karena dynamic facade saja terbukti belum cukup menurunkan route di bawah batas 8 KiB.

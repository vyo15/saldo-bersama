...
| Replay/duplicate write | Idempotency, nonce, timestamp, scoped hashed local limiter + durable Turso v13 rate-limit bucket lintas instance untuk mutation/external path | Authenticated read memakai local limiter agar tidak bergantung pada write lock; platform/WAF throttling tetap opsional defense-in-depth dan tidak boleh menggantikan idempotency atau authorization. |

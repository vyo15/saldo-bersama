...
| Replay/duplicate write | Idempotency, nonce, timestamp, scoped hashed local limiter + durable Turso rate-limit bucket lintas instance | Platform/WAF throttling tetap opsional defense-in-depth; tidak boleh menggantikan idempotency atau authorization. |

...
Schema column-level canonical merupakan hasil seluruh file berurutan di `database/migrations/`, saat ini dari `001_initial_schema.sql` sampai `022_goal_investment_funding.sql`. Dokumen ini menjelaskan arti dan lifecycle; bila ada perbedaan tipe/constraint, migration menang.
...
| `commitments` | Domain internal Kewajiban untuk KPR, cicilan, pinjaman, Arisan, atau kewajiban berkala lain beserta sisa pokok/setoran, rekening pembayaran, dan jadwal canonical. | Tinggi | Service/API; jadwal ditautkan 1:1 ke `recurring_rules`; aksi UI **Hentikan kewajiban** menonaktifkan record tanpa menghapus histori finansial |
| `commitment_movements` | Ledger progres Kewajiban untuk pembayaran, penerimaan Arisan, dan reversal; menyimpan pemisahan pokok/bunga ketika dapat ditentukan. | Tinggi | Dibentuk oleh service Kewajiban/Jadwal Rutin; tidak boleh ditulis langsung client |
| `savings_goals` | Target tujuan dengan nominal, tanggal, lifecycle, rekening compatibility, dan `funding_mode` `cash`/`investment`/`mixed`. | Sedang | Service/API; progress dapat berasal dari cash dan/atau investasi; hard delete hanya untuk target benar-benar belum dipakai |
...
- `recurring_rules.commitment_id`: nullable link 1:1 ke `commitments`. Jadwal yang memiliki link ini dikelola dari Kewajiban, tidak boleh diedit/dihapus terpisah dari Jadwal Rutin, dan future projection otomatis dihentikan saat Kewajiban selesai/dihentikan.
- `transactions.commitment_id` + `transactions.commitment_flow`: internal link transaksi aktual ke Kewajiban dengan flow `payment` atau `receipt`. Link ini hanya boleh dibuat oleh service Kewajiban/Jadwal Rutin agar ledger rekening dan `commitment_movements` tetap sinkron.
- `commitments.current_balance`: opening truth/sisa pokok atau setoran yang masih berjalan. Histori sebelum fitur dipakai tidak perlu dibackfill. **KPR tidak menginfer pokok flat** dari nilai awal + tenor; pembayaran KPR hanya mengubah `current_balance` bila `remaining_principal` aktual dikirim, sedangkan `installments_paid` tetap bergerak ketika occurrence selesai. Untuk tipe Cicilan/Pinjaman/Lainnya yang memang memiliki pola flat, pembayaran baru boleh membagi pokok tetap dan bunga/biaya otomatis; payload `remaining_principal` tetap menjadi override authoritative untuk koreksi/pelunasan. Arisan berkurang sesuai setoran. `received_amount` terpisah agar penerimaan Arisan tidak dianggap mengurangi histori setoran.
...
- receivable/settlement lintas pihak yang lebih umum di luar Kewajiban KPR/cicilan/pinjaman/Arisan: RFC-0012;

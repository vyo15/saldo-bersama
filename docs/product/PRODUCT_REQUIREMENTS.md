...
Alokasi Dana adalah wadah bernama yang dibuat user sejak awal. Nama Alokasi berbeda dari Kebutuhan: Kebutuhan adalah rincian kategori dan nominal di dalam wadah tersebut. Flow normal **tidak meminta budget awal**: Alokasi baru dibuat Rp0 setelah user memilih nama, rekening sumber, pengguna, periode, dan kebijakan sisa; detail Alokasi kemudian langsung menawarkan Tambah Kebutuhan. Pemasukan dapat dibagi melalui Alokasi Dana daily, weekly, biweekly, monthly, paycycle, atau custom; shared/personal; rollover; overspend policy; realokasi; sisa alokasi; dan dana belum dialokasikan. Alokasi bersifat account-bound: pendanaan Kebutuhan mengurangi `available_balance` dengan menaikkan `allocated_amount`, tetapi tidak mengubah saldo ledger fisik; transaksi aktual baru mengurangi saldo fisik dan sisa alokasi bersama-sama. Realokasi antar rekening wajib memakai transaksi Transfer. Penyesuaian manual tetap tersedia sebagai compatibility/pemulihan data lama, bukan langkah normal sebelum membuat Kebutuhan. Saat periode ditutup, rule aktif selalu memiliki periode aktif berikutnya. Policy `unallocated` melepaskan sisa lama, sedangkan `carry` hanya membawa sisa aktual; bila user memilih memakai lagi Kebutuhan, kebutuhan periode tujuan didanai otomatis dari Dana Tersedia dalam transaction yang sama. Pembuatan/pendanaan Alokasi tidak membuat transaksi ledger baru dan tidak menambah dana di luar saldo rekening.

### `REQ-PROD-05` Kebutuhan dalam Alokasi Dana — Partial

Alokasi Dana mendukung periodisasi harian sampai custom. Kebutuhan kategori dikelola dari detail Alokasi Dana dan memakai record budget serta relasi existing `budgets.envelope_rule_id`. Flow tambah dari detail Alokasi mendukung beberapa Kebutuhan sekaligus dalam editor compact; batch maksimal 20 item disimpan atomic, menolak kategori ganda pada Alokasi/batch yang sama, dan Jadwal Rutin opsional per item ikut rollback bila salah satu item gagal. Edit Kebutuhan existing tetap single-item. Kategori tetap master data bersama dan kategori yang sama boleh dipakai pada lebih dari satu Alokasi Dana; identitas Kebutuhan periode karena itu mencakup kategori, ownership, dan Alokasi Dana. Pemakaian nominal Kebutuhan hanya menghitung transaksi aktif pada kategori, ownership, periode, dan Alokasi Dana yang sama.

Nominal Kebutuhan adalah sumber pendanaan normal Alokasi. Create/batch Kebutuhan mengunci dana sebesar nominal baru; edit hanya menyesuaikan delta; hapus unused melepas dana penuh; arsip hanya melepas bagian nominal yang belum terpakai/dipesan; restore hanya mendanai ulang sisa yang belum terpakai. Nominal tidak boleh diturunkan di bawah pemakaian aktif. Seluruh penyesuaian terjadi pada `allocated_amount`/`available_balance`, bukan saldo ledger. Bila Dana Tersedia kurang, backend menolak mutation dengan `BUDGET_FUNDING_INSUFFICIENT` dan detail `requiredAmount`, `availableAmount`, `shortageAmount`; frontend tetap membolehkan draft disusun, menjelaskan kekurangan, dan menawarkan Tambah saldo tanpa menghilangkan draft, tetapi Kebutuhan belum disimpan sampai seluruh dana cukup. Detail Alokasi tetap dapat menunjukkan gap khusus data legacy yang dulu dibuat dengan funding manual dan menyediakan pemulihan eksplisit sekali.

Tidak ada surface Anggaran terpisah pada UI; `/anggaran` hanya compatibility redirect ke detail/overview Alokasi Dana agar tidak menduplikasi fungsi. Dashboard, laporan, dan push tetap memberi peringatan actionable ketika nominal Kebutuhan terlampaui. Saat menutup periode, user dapat memilih `Pakai lagi kebutuhan di periode berikutnya`. Sistem menyalin kategori dan nominal rencana Kebutuhan aktif yang belum ada di periode tujuan, mempertahankan rencana target yang sudah ada, lalu mendanai total kebutuhan periode tujuan sampai nominal rencana terpenuhi dengan memperhitungkan carry. Bila Dana Tersedia tidak cukup, seluruh close/copy/funding di-rollback; transaksi dan histori pemakaian lama tidak pernah disalin.

...
Global **`+ Catat`** pada app shell adalah launcher aktivitas, bukan mutation layer baru. `Pengeluaran`, `Pemasukan`, dan `Transfer` mendelegasikan ke composer transaksi canonical; `Bayar kewajiban` mendelegasikan ke occurrence Jadwal Rutin managed (`recurring.payOccurrence`); `Setor ke target` membawa user ke flow Target → Alokasi canonical; dan `Investasi` membuka flow pencatatan investasi manual yang tidak memindahkan saldo rekening. Entry point kontekstual dari Kewajiban atau Alokasi harus memakai workflow yang sama agar satu kejadian uang tidak dicatat dua kali.

...
Global **`+ Catat`** pada app shell adalah launcher aktivitas, bukan mutation layer baru. Launcher menanyakan satu kali apa yang baru terjadi: `Pengeluaran`, `Pemasukan`, `Transfer`, `Bayar kewajiban`, `Target`, atau `Investasi`. Setelah `Pengeluaran`/`Pemasukan`/`Transfer` dipilih, composer transaksi canonical menerima jenis yang sudah dikunci, judul/CTA kontekstual, dan tidak menanyakan jenis transaksi untuk kedua kalinya; tombol kembali membawa user ke launcher tanpa membuat mutation. `Bayar kewajiban` mendelegasikan ke occurrence Jadwal Rutin managed (`recurring.payOccurrence`); `Target` membawa user ke flow Target → Alokasi canonical; dan `Investasi` membuka flow pencatatan investasi manual yang tidak memindahkan saldo rekening. Refund tetap capability transaksi tetapi bukan quick action global karena pengembalian lebih tepat dimulai dari konteks transaksi asal. Entry point kontekstual dari Kewajiban atau Alokasi harus memakai workflow yang sama agar satu kejadian uang tidak dicatat dua kali.

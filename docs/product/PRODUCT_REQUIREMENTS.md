...
Composer transaksi mobile bersifat progressive: pada entry generic jenis transaksi tetap dapat dipilih, sedangkan entry dari `+ Catat`/planning mengunci jenis yang sudah diketahui. Jalur rapid-entry mengutamakan nominal, rekening, kategori, **Penggunaan dana**, lalu tanggal (default hari ini) yang tetap terlihat; metode pembayaran dan catatan berada pada disclosure opsional **Catatan & metode pembayaran**. Untuk expense, tepat satu Kebutuhan yang cocok pada rekening/kategori/periode dihubungkan otomatis namun tetap terlihat dan dapat diubah; bila beberapa Kebutuhan cocok, user wajib memilih salah satu atau **Dana Tersedia**; bila tidak ada yang cocok, UI langsung menampilkan **Dana Tersedia · belum terkait Alokasi** dan transaksi dapat disimpan tanpa konfirmasi kedua. **Belum memilih** hanya berlaku pada kondisi ambigu dan tidak boleh diam-diam dianggap Dana Tersedia. Context transaksi yang dibuka dari row Kebutuhan membawa `budget_id` + `envelope_period_id` + kategori + rekening dan tidak boleh hilang hanya karena ada Kebutuhan lain berkategori sama. Perubahan rekening/kategori/tanggal membatalkan link yang menjadi stale dan menjalankan smart matching ulang. Composer transaksi tidak menyediakan pemilihan Alokasi Dana manual terpisah karena relasi operasional harus berasal dari Kebutuhan yang dipilih. Sebelum Simpan, UI boleh menampilkan **perkiraan lokal** dampak terhadap Kebutuhan, Dana Tersedia, dan saldo rekening, termasuk delta edit `kondisi kini - dampak lama + dampak baru`; preview harus menyebut sumber pengurangan dana sehingga Kebutuhan vs Dana Tersedia tidak ambigu; server tetap authority final. Setelah create berhasil, ringkasan sukses memakai snapshot `dashboard.overview` yang telah disegarkan bila tersedia. Konfirmasi duplicate, overspend guard/alasan, period lock, dan transfer approval **tetap dipertahankan**; hanya konfirmasi khusus unallocated yang dihapus karena keputusan penggunaan dana sudah terlihat sebelum submit.

Global **`+ Catat`** pada app shell adalah launcher aktivitas, bukan mutation layer baru. Launcher menanyakan satu kali apa yang baru terjadi: `Pengeluaran`, `Pemasukan`, `Transfer`, `Bayar kewajiban`, `Target`, atau `Investasi`. Setelah `Pengeluaran`/`Pemasukan`/`Transfer` dipilih, composer transaksi canonical menerima jenis yang sudah dikunci, judul/CTA kontekstual, dan tidak menanyakan jenis transaksi untuk kedua kalinya; tombol kembali membawa user ke launcher tanpa membuat mutation. **Otomatisasi hanya dipakai ketika konteks pasti**: entry point eksplisit atau tepat satu pilihan valid boleh diisi otomatis dan tetap terlihat; histori hanya mengurutkan opsi sebagai rekomendasi dan tidak boleh memilih rekening, kategori, sumber, Target, atau aset secara diam-diam. `Bayar kewajiban` mendelegasikan ke occurrence Jadwal Rutin managed (`recurring.payOccurrence`) dan satu occurrence valid boleh dilanjutkan langsung; bila beberapa tersedia user memilih. `Target` langsung membawa pilihan Target ke flow pendanaan canonical—satu Target aktif boleh dilanjutkan otomatis, banyak Target meminta pilihan—dan quick-record memulai nominal tanpa asumsi bila tidak ada suggested amount eksplisit. Pendanaan dapat memakai Rekening, Investasi, atau campuran; Buy/Sell yang ditautkan ke Target tetap satu kejadian investasi dan tidak membuat mutasi Target duplikat. `Investasi` membuka flow pencatatan manual canonical; satu portofolio operasional boleh dipilih otomatis, beberapa portofolio meminta pilihan, sedangkan aset/kuantitas/harga tetap dipilih atau diisi di dialog Investasi dan tidak mengubah saldo rekening otomatis. Refund tetap capability transaksi tetapi bukan quick action global karena pengembalian lebih tepat dimulai dari konteks transaksi asal. Entry point kontekstual dari Kewajiban atau Alokasi harus memakai workflow yang sama agar satu kejadian uang tidak dicatat dua kali.
...
Surface **Atur Dana** menampilkan total compact **Dana yang bisa dialokasikan** lintas rekening sebagai konteks aksi, tetapi angka tersebut **bukan satu kolam uang virtual** dan bukan pengganti hero **Saldo Keluarga** + konteks **Dana yang bisa kamu gunakan** di Beranda. Breakdown rekening harus tersedia, dan setiap Alokasi tetap mempunyai tepat satu rekening sumber. Saat sudah ada Alokasi aktif, di bawahnya ada dua entry point yang tetap eksplisit: primary **Alokasikan dana** untuk menambah dana ke Alokasi existing dan secondary **Alokasi baru** untuk membuat wadah baru. Pada true-empty, satu CTA **Buat Alokasi pertama** cukup karena belum ada tujuan yang dapat menerima dana. Halaman utama tidak memakai tab domain `Alokasi/Jadwal Rutin/Kewajiban`; satu daftar **Aktif** menjadi presentation layer yang menggabungkan objek pengeluaran tersebut. Jika Jadwal Rutin expense atau Kewajiban sudah tertaut ke Kebutuhan/Alokasi aktif, item tidak boleh diduplikasi sebagai row terpisah; standalone expense recurring/Kewajiban tetap dapat tampil dan membuka detail canonical masing-masing. Pemasukan rutin tidak tampil sebagai rencana Atur Dana karena transaksi pemasukan yang sudah tercatat langsung menambah saldo rekening. Flow generik **Alokasikan dana** memilih rekening sumber lebih dahulu ketika dibutuhkan lalu hanya menawarkan Alokasi yang memakai rekening tersebut. Row Aktif hanya menampilkan signal minimum yang benar dan seluruh row membuka detail; informasi teknis/kompleks ditunda ke halaman detail. Penyesuaian dana contextual hanya muncul dari detail, attention/shortage, atau workflow yang memang sudah mengetahui rekening sumber + target. Rekening Bersama dapat membuat Alokasi untuk Bersama atau anggota yang diizinkan capability backend; rekening personal mengunci penerima ke pemilik rekening. Dana lintas rekening tidak boleh dicampur diam-diam: realokasi lintas rekening tetap wajib melalui Transfer.
...
Pada daftar Kebutuhan, state 100% mengikuti pola pemakaian, bukan satu warning generik: **Sekali bayar** yang terpakai tepat sebesar rencana ditampilkan sebagai **✓ Selesai** dan menjadi row compact tanpa quick action pencatatan; `flexible`/`recurring` yang mencapai 100% tetap aktif tetapi berstatus **Dana habis** dan tidak menawarkan pencatatan baru sampai rencana/dana diubah; nilai di atas 100% tetap **Melebihi rencana**. Metadata historis seperti `Terpakai` dan waktu penyelesaian tidak memenuhi row selesai dan tersedia pada **Detail kebutuhan**. Kebutuhan sekali bayar yang selesai tepat tidak membuat alert/notifikasi ambang 100%, sedangkan overspend tetap diperingatkan.
...
Target menyimpan nominal, tanggal, prioritas, serta `funding_mode` **cash / investment / mixed**. Cash Target memakai movement + transaksi transfer canonical. Investasi Target memakai `goal_investment_events` append-only untuk mengalokasikan sebagian holding, Buy/Sell terkait Target, release, dan hasil jual yang tetap dipertahankan tanpa menciptakan transaksi saldo kedua. `current_amount` adalah satu progress authoritative: cash movement + market value porsi investasi + retained sale cash; modal investasi dan hasil investasi tetap konteks sekunder. Kenaikan harga tidak mengubah Target menjadi selesai otomatis—completion tetap aksi eksplisit user.

Satu holding boleh dibagi ke beberapa Target selama total saham yang dialokasikan tidak melebihi holding aktual. Entry `Target → Tambah dana → Investasi` mempertahankan konteks Target sampai Buy/setup selesai; posisi investasi pertama + alokasi Target dilakukan atomik. Sell pada porsi Target dapat mempertahankan hasil jual untuk Target atau melepasnya. Tidak ada flow Target → Alokasi dan tidak ada pencatatan progress ganda antara movement Target dengan event investasi.
...
Menampilkan **Saldo Keluarga** sebagai angka transparansi utama Beranda: total rekening operasional non-investasi yang dapat dilihat kedua pasangan, tanpa menyiratkan bahwa seluruh nominal bebas digunakan user aktif. **Dana yang bisa kamu gunakan** (`safeToSpend`) menjadi konteks operasional langsung di hero yang sama, sedangkan ownership **Saya / Pasangan / Bersama**, **Aman dipakai / hari**, dan **Sisa di Alokasi** menjelaskan sumber serta batas penggunaan uang secara ringkas. Nilai investasi/RDN tidak masuk Saldo Keluarga.

Pada mobile, hierarchy canonical bersifat decision-first dan compact: header identitas menampilkan **Hai, {nama}** + **Diperbarui HH.mm**, lalu hero **Saldo Keluarga** sebagai angka transparansi utama dengan **Dana yang bisa kamu gunakan** sebagai angka operasional. Tepat di bawah hero ada satu panel tiga kolom **Saya / Pasangan / Bersama**; kolom memakai nominal compact, menjelaskan capability (`Bisa digunakan`, `Hanya dilihat`, `Dipakai bersama`), dan membuka halaman Rekening dengan ownership filter yang sesuai. **Aman dipakai / hari** + **Sisa di Alokasi** tetap dua metrik ringkas. **Perlu dilakukan** hanya boleh muncul di Beranda untuk alert yang benar-benar mendesak (selisih rekonsiliasi aktif atau jadwal terlambat); threshold Kebutuhan/Alokasi, funding gap, goal pace, stale reconciliation, dan reminder biasa tetap berada di Notification Center agar Beranda tidak berubah menjadi daftar warning. Setelah itu hierarchy berlanjut setup bila prerequisite belum lengkap → shortcut **Rekening / Target / Investasi** → **Rencana terdekat** (maksimal satu Kebutuhan prioritas + satu Jadwal terdekat) → Aktivitas terbaru → Investasi hanya bila sudah ada aset/nilai tercatat. Tidak ada standalone card `Insight Keuangan` atau card positif “semua aman”. `Atur Dana` adalah label pengalaman untuk workspace Perencanaan dan menjadi heading halaman agar intent user tetap konsisten. Root workspace memakai satu daftar **Aktif** untuk pengeluaran; `Alokasi Dana`, `Jadwal Rutin expense`, dan `Kewajiban` tetap domain canonical di belakang layar dan tersedia sebagai detail/sub-route, bukan tiga tab sejajar yang harus dipahami user. Navigasi bawah mobile canonical adalah **Beranda · Atur Dana · CATAT · Transaksi · Lainnya**; `Laporan` dan `Notifikasi` tersedia di `Lainnya`, sedangkan tombol **CATAT** tetap menjadi entry point global untuk aktivitas uang. Membuka Notifikasi dari `Lainnya` membawa konteks halaman asal untuk Back; direct/deep-link tetap memiliki fallback Beranda dan tidak menambah item bottom navigation. Navigasi Back mengikuti hierarchy, bukan history generik: root tidak menampilkan Back; detail/sub-route menampilkan parent kontekstual yang deterministic; modal root memakai `×` dan sub-step modal memakai `←`. Bottom navigation tetap menunjukkan domain induk pada child route, sementara direct/deep-link selalu memiliki fallback parent aman.

Dana yang bisa kamu gunakan di Beranda sudah memperhitungkan capability rekening, proteksi, sisa Alokasi Dana, dan komitmen Jadwal Rutin operasional yang belum tercakup Alokasi; Jadwal Rutin yang tertaut ke Kebutuhan dalam Alokasi tidak boleh dikurangi kedua kali. Uang personal pasangan tidak masuk angka operasional user aktif, sementara rekening Bersama tetap dapat ikut sesuai capability. Saldo RDN/market value tidak boleh masuk Saldo Keluarga.
...
### `REQ-PROD-14` Pastikan Saldo Sesuai (rekonsiliasi internal) — Implemented

Menyimpan saldo sistem, saldo aktual, selisih, status, catatan, dan actor untuk rekening non-Investasi. Aksi user-facing **Pastikan Saldo Sesuai** menempel pada Rekening; Dashboard tidak menyediakan shortcut permanen, sedangkan Notification Center dapat memberi pengingat setelah melewati cadence yang dikonfigurasi (default 30 hari). User memasukkan saldo aktual satu kali lalu sistem membandingkan dengan saldo tercatat. Hasil pemeriksaan—termasuk mismatch—menjadi checkpoint sehingga selisih tetap berada di histori tanpa terus menghasilkan notifikasi aktif. Jika ada selisih, UI menawarkan pencatatan transaksi yang tertinggal atau pemeriksaan aktivitas rekening terkait tanpa membuat adjustment otomatis. Rekening Investasi/RDN memakai `investments.reconciliations.create` karena Saldo RDN dan holding harus diverifikasi bersama.
...
Notification Center bersifat task-first: kondisi actionable ditampilkan sebagai **Perlu tindakan**, event lain sebagai **Lainnya**, item dengan timestamp terbaru selalu berada paling atas di kelompok/filter masing-masing, dan deep-link membawa konteks entity/tindakan yang sudah diketahui. Status **belum dibaca** harus berbeda jelas secara visual dari item yang sudah dibaca (indikator, background, dan hierarchy typography), sedangkan status baca canonical tetap disimpan server per actor sebagai `notification_key + fingerprint` sehingga HP/Desktop sinkron; **read tidak sama dengan resolved**. Source alert tetap menentukan apakah kondisi aktif, dan fingerprint baru membuat kondisi yang berubah/memburuk muncul sebagai unread kembali. Preference tujuh tipe alert tetap account-level, `recurring_completed` default mati agar tidak berisik, pengingat manual one-shot tetap tersedia, dan cadence rekonsiliasi dapat dipilih 0/14/30/60 hari (default 30). Push hanya aktif bila VAPID lengkap.
...
**Setup usable = Rekening + Kategori siap → user sudah dapat mencatat transaksi. Atur Dana bersifat opsional: buat wadah Alokasi Dana → tambahkan Kebutuhan → sistem mengikat Dana Tersedia otomatis sesuai nominal Kebutuhan → jadwalkan bila rutin. Target adalah jalur tujuan terpisah dari Alokasi: user memilih menabung melalui Rekening, Investasi, atau campuran; hanya cash Target yang membuat transfer ledger, sedangkan investasi memperbarui progress melalui event investasi/nilai pasar tanpa transaksi saldo duplikat. Transaksi aktual tetap menjadi sumber ledger, lalu dibandingkan dengan Kebutuhan/anggaran, direkonsiliasi, dan ditutup per periode sesuai blocker canonical.**

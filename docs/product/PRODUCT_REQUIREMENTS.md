...
Recurring rule/occurrence mendukung nominal, frekuensi, jatuh tempo, rekening, metode pembayaran, priority, payment/reversal, overdue, status pembayaran, serta **skip/restore satu occurrence**. Untuk pembayaran expense, Jadwal Rutin dapat ditautkan ke tepat satu Kebutuhan melalui `budget_id`: satu kandidat kompatibel boleh dipilih otomatis, beberapa kandidat wajib meminta pilihan user, dan nol kandidat tetap sah sebagai pembayaran mandiri. Link tersebut mencegah Dana Tersedia mencadangkan pembayaran yang dananya sudah disiapkan melalui Alokasi untuk kedua kali. UI tidak lagi memiliki penanda Auto-debit. Saat occurrence jatuh tempo, sistem menempatkannya sebagai transaksi rutin yang perlu dikonfirmasi; saldo/ledger baru berubah setelah nominal aktual disimpan. Kolom `auto_debit` legacy dipertahankan hanya untuk kompatibilitas data lama dan seluruh write baru menetapkannya `false`, termasuk Jadwal yang dikelola Komitmen.

Komitmen (KPR, cicilan, pinjaman, Arisan) tetap menjadi domain kewajiban, bukan Target. Satu Komitmen membuat tepat satu Jadwal Rutin managed; form Komitmen dapat menautkan Jadwal managed tersebut ke Kebutuhan kompatibel memakai rule yang sama. `budget_id` disimpan pada `recurring_rule`, bukan diduplikasi pada tabel Komitmen. Jadwal managed tetap dibayar melalui `recurring.payOccurrence`, sehingga progres kewajiban, occurrence berikutnya, ledger, dan reversal tetap canonical.

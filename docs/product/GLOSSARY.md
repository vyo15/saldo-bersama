...
| Atur Dana | Nama user-facing untuk workspace yang secara internal masih memakai route/modul `perencanaan` demi compatibility. Istilah ini **bukan** domain finansial baru dan tidak menggantikan `Alokasi Dana`; di dalamnya user tetap memakai tab/domain Alokasi Dana, Jadwal Rutin, dan Kewajiban. Pada tab Alokasi, total `Dana yang bisa dialokasikan` lintas rekening hanya konteks aksi dan bukan satu pool uang baru. |

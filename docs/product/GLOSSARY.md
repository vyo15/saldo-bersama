...
| Dana yang bisa kamu gunakan | Label user-facing untuk read-model `safeToSpend`: dana operasional non-investasi yang masih dapat dipakai actor setelah capability rekening, proteksi, Alokasi Dana, dan komitmen Jadwal Rutin operasional yang belum tercakup Alokasi. Jadwal yang sudah ditautkan ke Kebutuhan dalam Alokasi tidak dikurangi kedua kali. Ini konteks operasional di hero Beranda, bukan Saldo Keluarga dan tidak memasukkan uang personal pasangan. |
...
| Atur Dana | Nama user-facing untuk workspace pengaturan **uang keluar** yang secara internal masih memakai route/modul `perencanaan` demi compatibility. Istilah ini **bukan** domain finansial baru dan tidak menggantikan `Alokasi Dana`. Halaman utama memakai satu daftar **Aktif**: Alokasi, Jadwal Rutin expense, dan Kewajiban yang saling tertaut ditampilkan sebagai satu objek user-facing tanpa duplikasi, sedangkan detail domain tetap tersedia saat dibuka. Pemasukan tidak dikonfigurasi ulang di sini; transaksi pemasukan yang sudah tercatat langsung menambah saldo rekening. Total `Dana yang bisa dialokasikan` lintas rekening hanya konteks aksi dan bukan satu pool uang baru. |
...
| Kebutuhan | Rencana nominal kategori di dalam satu Alokasi Dana. Menambah/mengubah Kebutuhan otomatis menyesuaikan dana Alokasi dari Dana Tersedia bila aman; menurunkan atau mengarsipkan hanya melepas sisa yang belum terpakai/dipesan dan tidak menyapu buffer, sedangkan Kebutuhan yang benar-benar belum pernah dipakai dapat dihapus permanen setelah preview lifecycle. Jadwal/Komitmen expense yang menautkan `budget_id` memakai dana yang sudah disiapkan sehingga tidak dicadangkan kedua kali. Implementasi internal memakai `budgets` + `envelope_rule_id`. |

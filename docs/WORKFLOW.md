...
`npm run verify` melakukan preflight runtime Node yang didukung dan dependency yang sudah terpasang, lalu menjalankan source validation, lint/syntax, frontend regression, production build, build budget, rendered browser smoke, serta seluruh backend regression satu kali dengan coverage. Guard security/governance sudah berada di suite frontend/backend sehingga tidak ada re-run `test:guard` terpisah. Ia tidak menjalankan `npm ci` atau menghapus dependency. `npm run zip` memakai gate yang sama, tetapi sebelum verifikasi ia boleh menjalankan bootstrap `npm ci --include=dev` satu kali bila probe dependency development canonical menemukan dependency yang belum tersedia. Browser smoke saat ini tidak membutuhkan `axe-core`; pemeriksaan aksesibilitas rendered yang aktif adalah overflow/reflow dasar, keyboard focus, WCAG text-spacing, dan reduced-motion. Probe pasca-install dijalankan lewat proses Node segar supaya cache resolver dari kondisi sebelum install tidak menghasilkan false-negative.

Rendered browser smoke kini kembali menjadi bagian quality gate melalui `scripts/browser-smoke.mjs` tanpa dependency browser-test tambahan. Smoke memakai Chrome/Chromium/Edge lokal (atau `CHROME_PATH`/`CHROME_BIN`) terhadap production build dan memeriksa login publik pada viewport canonical, page-level overflow, focus rendered, WCAG text-spacing, serta reduced-motion. Startup browser memakai port DevTools yang dipilih browser (`--remote-debugging-port=0` + `DevToolsActivePort`) agar CI tidak bergantung pada port acak yang dapat bentrok; bila browser gagal start, log proses terakhir ikut ditampilkan supaya failure GitHub Actions dapat didiagnosis. Pemeriksaan focus wajib memakai input keyboard CDP (`Tab`) dan `:focus-visible`, bukan `element.focus()` programatik, karena programmatic focus tidak menjamin modality keyboard dan dapat menghasilkan silang palsu berbeda antara Chrome Windows dan runner Linux. Regression tooling menjaga kontrak ini supaya bug yang sama tidak masuk kembali. Smoke authenticated deterministic memakai fixture test-only untuk memeriksa shell/route utama tanpa membuka auth bypass production. Journey login nyata, data production-like, screen reader, Firefox/Safari, installed-PWA, dan real-device tetap memakai manual QA.

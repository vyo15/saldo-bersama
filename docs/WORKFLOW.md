...
Jika build budget gagal, jangan ubah threshold sebagai shortcut. Audit route chunk, static dependency import, CSS global, dan asset tidak terpakai. Firebase popup mobile adalah lazy provider chunk yang dipreload sebelum interaksi agar popup tetap berada dalam user gesture; desktop GIS tetap terpisah. Perubahan auth mobile wajib ikut security/deployment regression dan tidak boleh memindahkan authorization dari backend. Verification wrapper selalu membersihkan generated build/test output sesudah PASS maupun gagal, sehingga retry dimulai dari artefak bersih tanpa menghapus dependency atau env lokal.

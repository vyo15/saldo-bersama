...
`npm run prod` tidak menarik atau menulis Development. Command ini memastikan `.env.production.local` tersedia/lengkap, membaca `.env.local` hanya untuk membuktikan isolasi DEV/PROD, boleh menyejajarkan hanya grup Google bridge pusat bila Production lokal kosong, menguji Turso Production secara read-only, lalu memeriksa Vercel Production melalui health, anonymous `/api/session`, dan frontend shell sebelum membuka Production pada terminal interaktif. Anonymous session yang sehat harus menjawab `401 UNAUTHENTICATED`; response `5xx` tetap blocker walaupun health aggregate hanya memiliki warning. Database/schema/binding, maintenance, dan integrity failure tetap blocker; scheduler/integrasi/backup/notifikasi degraded menjadi operational warning hanya bila endpoint session dan frontend tetap sehat.

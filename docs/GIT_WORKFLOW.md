...
`npm run prod` tidak menulis Development. Command ini auto-link ke project Vercel canonical, auto-sync environment `production` ke `.env.production.local`, mencoba `vercel env run -e production` untuk key runtime yang tidak tersedia lewat export biasa, lalu membaca `.env.local` hanya untuk membuktikan isolasi DEV/PROD. Sesudah itu Turso Production diuji read-only dan Vercel Production + frontend shell diperiksa. Database/schema/binding, maintenance, dan integrity failure tetap blocker; scheduler/integrasi/backup/notifikasi degraded menjadi operational warning.

Workstation tidak lagi meminta copy manual Turso Production sebagai workflow normal. `.env.production.local` adalah mirror gitignored yang diperbarui dari Vercel Production. Bila `vercel env pull` mengembalikan placeholder Sensitive, tooling mencoba `vercel env run -e production`; bila Vercel CLI tetap tidak menyediakan plaintext untuk key wajib, operasi berhenti fail-closed dan tidak menyalin credential Development.

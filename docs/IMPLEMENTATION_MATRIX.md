...
| `REQ-PROD-02` | Transaksi lengkap | Partial | finance service/TransactionForm + `MobileTransactionHistory` history-first UI + mobile grouped composer/same-sheet selection + smart source-account picker + Kebutuhan→Alokasi suggestion + compact after-transaction feedback | participant roles, receipt, draft, debt pending RFC-0011/0012 |
...
| `REQ-PROD-11` | Quick/draft transaction | Partial | quick form, duplicate guard, unallocated review queue, `Pakai lagi` safe prefill, kategori sering dipakai per rekening + search pada selection view, `Tambah lagi` dengan intent baru, semuanya tanpa auto-submit | draft/template tersimpan pending RFC-0011 |

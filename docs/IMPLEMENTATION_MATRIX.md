...
| `REQ-A11Y-001` | Accessibility baseline | Partial | semantic/static regression, AA semantic danger-pair guard, native text/date controls 16px, app-owned `SelectionField` default/compact/embedded + search target mobile ≥44px + `aria-expanded`/`aria-pressed`, meaningful financial micro-text ~12px+, top/bottom safe-area contract, focus-visible + touch pressed state, visual-scrollbar hiding tanpa scroll lock, non-color active navigation indicator, notification preference descriptions + `aria-describedby` | axe penuh; real-device coverage pending untuk Chrome/Firefox/Safari dan installed-PWA |
| `REQ-PROD-01` | Rekening/sumber uang | Partial | accounts/read models/dashboard + schema v5 account number/`bank_template` + schema v8 `ewallet_template` + schema v9 `assignee_user_id` pada Alokasi + list/detail financial-card UI + mobile horizontal card carousel + split balance summary/recent activity + header Rekening/info/Tambah + quick actions Riwayat/Transfer/Kelola tanpa duplikasi + single-root rounded e-wallet clipping + internal overscan anti-fringe + canonical 1024×645 investment card asset + analitik diarahkan ke Riwayat + label/capability Administrator/Member + route rekening terpisah | Mode privacy granular pending RFC-0015; real-resource Administrator/Member smoke pending |
...
| `REQ-PROD-04` | Alokasi Dana | Implemented | `envelope_rules`/`envelope_periods` internal + add/release alokasi existing tanpa ledger + flow buat bernama `Nama → rekening → penerima → periode → dana` + helper estimasi kategori + preview dana tersedia + overview CTA hierarchy `Alokasi baru → Tambah kebutuhan/Atur dana` sesuai capability + detail unified master-card (hero/metrics → Kebutuhan → Jadwal Terkait) + funding flow dari dashboard/pemasukan + close selalu menyiapkan periode aktif berikutnya + release-to-Target prefill + archive/restore rule + reverse reallocation | Full device regression setelah patch terbaru |
| `REQ-PROD-05` | Kebutuhan + overview Anggaran | Partial | Kebutuhan memakai `budgets` + `envelope_rule_id`; kategori dapat dipakai lintas Alokasi Dana; detail Alokasi memakai grouped rows dengan ikon kategori/status/progress dan agregat terpakai; copy opt-in ke periode berikutnya tidak menimpa target existing; `/anggaran` read-only overview + alerts | Rule Kebutuhan multi-periode/recurring independen belum ada |
...
| `REQ-PROD-09` | Dashboard pasangan | Implemented | shared dashboard view model; mobile hanya menampilkan satu next action prioritas + bell unread, desktop menautkan jumlah notifikasi aktif, keduanya menuju `/notifikasi`; free funds dan expense tanpa Alokasi Dana dipisah; CTA alokasi/Target; onboarding compact; mobile/desktop memakai `investments.overview` yang sama untuk ringkasan portfolio | Production/device smoke |
...
| `REQ-PROD-14` | Rekonsiliasi | Implemented | reconciliation service + alerts + signed actual balance untuk rekening `allow_negative`; attention hanya memprefill rekening, flow explicit same/different baru meminta actual balance bila perlu, completion one-way ke Beranda/CTA transaksi tanpa auto-adjustment atau second-submit | Cadence configurable belum ada |
...
| `REQ-PROD-16` | Notifikasi | Partial | tujuh tipe alert otomatis + in-app Notification Center `/notifikasi` dari `dashboard.overview.alerts` dengan flat rows, local unread state ber-TTL, shared deep-link guidance, mobile bell badge, desktop link; preference per user + manual reminder one-shot + Web Push delivery/retry tetap terpisah di `/pengaturan/notifikasi` | In-app durable event history/server read receipts belum ada; real Android/iOS masih pending; transaksi besar/saldo rendah/cadence tambahan belum ada |
...
| `REQ-PROD-20` | Investasi manual berbasis RDN | Implemented | schema v16; `services/investments.js`; action policy/registry/security; route `/investasi`; continuation RDN; opening position + Cash RDN awal; canonical investment account artwork; fixed prototype LQ45 stock catalog + logo picker tanpa manual instrument form; weighted cost basis; valuation; P/L; reconciliation/correction; multi-RDN labels; backup/integrity tests | Production migration + real-resource RDN/broker reconciliation + device smoke tetap guarded |

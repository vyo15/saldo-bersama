...
| `REQ-A11Y-001` | Accessibility baseline | Partial | semantic/static regression, AA semantic danger-pair guard, native text/date controls 16px, app-owned `SelectionField` target ≥44px + `aria-expanded`/`aria-pressed`, meaningful financial micro-text ~12px+, top/bottom safe-area contract, focus-visible + touch pressed state, visual-scrollbar hiding tanpa scroll lock, non-color active navigation indicator, notification preference descriptions + `aria-describedby` | axe penuh; real-device coverage pending untuk Chrome/Firefox/Safari dan installed-PWA |

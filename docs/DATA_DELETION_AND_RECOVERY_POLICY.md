...
| Komitmen (KPR/cicilan/pinjaman/Arisan) | Arsipkan; jadwal managed ikut dihentikan | Reversal pembayaran memulihkan progres/jadwal bila completion otomatis dibatalkan | Ledger `commitment_movements` tidak dihapus oleh lifecycle harian; hanya future occurrence reproducible dari jadwal managed yang boleh dibersihkan |
...
- **Ephemeral/security session state:** `idempotency_keys`, `request_nonces`, `rate_limit_buckets`, `import_previews`, `restore_previews`, dan `user_sessions`. Cleanup expired existing tetap diperbolehkan sesuai guard saat ini; khusus `user_sessions`, source mempertahankan revoked/expired session selama 30 hari sebelum hard delete. Session aktif tidak eligible dan audit keamanan yang terpisah tidak ikut dipurge.

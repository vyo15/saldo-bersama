...
- Tepat di bawah carousel, rekening mobile **non-investasi** menampilkan satu summary finansial terpisah: **Dana tersedia** sebagai nominal utama, lalu tiga kolom compact **Saldo / Dialokasikan / Tersedia** dengan contextual help canonical dan privacy toggle. Bila rekening aktif adalah Investasi/RDN, summary berganti menjadi **Saldo RDN** + konteks `Tujuan dana Investasi` dan tidak menampilkan Dialokasikan/Tersedia operasional. Aksi cepat di bawahnya tetap **Riwayat / Transfer / Kelola (atau Detail)** dalam satu baris; `Transfer` hanya ada di area ini agar header tidak memuat CTA ganda. `Transaksi terbaru` tampil sebagai surface informasi berikutnya. Grafik/analitik pengeluaran tidak ditempatkan di layar utama Rekening dan diarahkan ke Riwayat/route Transaksi yang memang memiliki presentation analitik canonical.
...
- Komponen presentasi desktop/mobile wajib memakai domain/read model canonical yang sama ketika menampilkan data yang sama. Kontrol presentasi boleh berbeda bila dataset yang tersedia memang berbeda cakupan; Dashboard mobile hanya menampilkan tiga Aktivitas Terbaru dari recent slice, sedangkan search/filter lengkap tetap di `/transaksi`. Shortcut utama Beranda mobile adalah `Alokasi Dana`, `Jadwal Rutin`, `Target`, dan `Rekening`; transaksi baru tetap memakai tombol `+` global agar Beranda tidak menduplikasi composer transaksi. Setelah quick actions, hierarchy compact adalah insight kondisi → satu Rencana Keuangan prioritas → satu Jadwal Terdekat → Aktivitas Terbaru → ringkasan Investasi bila tersedia. `AccountVisual`, detail Alokasi, dan detail arus kas tidak diduplikasi pada Beranda mobile karena masing-masing sudah memiliki route canonical. Jika ada alert aktif, Dashboard mobile menampilkan satu `Perlu dilakukan` prioritas sebelum setup/quick actions dan seluruh alert lain tetap dapat dijangkau melalui `/notifikasi`. Transfer baru pada viewport mobile tetap memakai `TransactionForm` canonical dengan presentasi `mobile-transfer`. Business form tidak boleh diduplikasi hanya untuk perangkat berbeda.

...
- Rekening Investasi/RDN tidak memakai generic account reconciliation. Alert RDN membuka `/investasi` dan workflow Cocokkan Investasi untuk memverifikasi Saldo RDN + holding.
...
- Seluruh pilihan app-owned berbasis daftar memakai `frontend/src/components/common/SelectionField.jsx` atau selection view feature yang sudah canonical. Native `<select>` tidak dipakai lagi pada `frontend/src` karena popup browser/OS memutus bahasa visual aplikasi dan tidak dapat distyle konsisten.
- `SelectionField` memakai button trigger + option buttons dengan `aria-expanded`/`aria-pressed`, Escape/outside-dismiss, optional search untuk taxonomy panjang, optional option-groups, selected check, dan target sentuh minimal 44px. Search pada daftar panjang boleh sticky agar tetap terjangkau ketika options di-scroll. Mode visual `compact`/`embedded` boleh lebih rapat pada desktop, tetapi pada `<=820px` trigger, search, dan option tetap memakai effective hit target `--mobile-control-height` 44px. Pada mobile daftar berkembang inline dalam form/sheet yang sama agar tidak membuat nested modal; desktop boleh memakai popover anchored.
...
- Pilihan fixed yang sedikit tetap memakai `VisualChoiceGroup`/choice chips. Keputusan explanatory dua-opsi yang membutuhkan satu kalimat penjelasan memakai mode `descriptive`: tile harus sejajar/equal-height, icon badge satu bahasa visual, selected check eksplisit, label boleh wrap, description maksimal dua baris, dan helper panel hanya dipakai bila ada konsekuensi yang memang perlu dijelaskan. Pilihan identitas/ownership yang padat tetap memakai mode `compact`, bukan card besar. Search hanya ditambahkan bila jumlah item atau taxonomy memang panjang; metadata sekunder hanya boleh tampil bila membantu keputusan (mis. dana tersedia rekening atau sisa Alokasi Dana).
...
- Buat Alokasi Dana mengikuti urutan `Nama alokasi → Ambil dana dari → Digunakan oleh → Periode dan sisa → Dana yang disiapkan`. Nama adalah identitas wadah dan wajib diisi user; Kebutuhan tetap rincian kategori di dalam Alokasi. Bila user belum tahu nominal, action-row `Belum tahu nominalnya?` membuka step `Susun kebutuhan` pada modal yang sama dan menghitung dari Kategori pengeluaran aktif tanpa nested modal atau auto-save Kebutuhan.
...
- **Logo-first hanya untuk surface compact pemilihan rekening.** Bank canonical BCA/BNI/BTN/Mandiri/Permata dan E-wallet ShopeePay/DANA/GoPay/OVO/LinkAja memiliki asset logo WebP persegi 256×256 dengan alpha transparan pada `bank-logos/` dan `ewallet-logos/`. `SelectionField`/selection view rekening memakai logo ini; rekening non-brand tetap memakai semantic account-type icon. Artwork `bank-cards/`/`ewallet-cards/` tetap canonical untuk `AccountVisual`, carousel rekening, detail, dan preview template sehingga kartu rasio 1.586:1 tidak dipaksa menjadi thumbnail kotak.

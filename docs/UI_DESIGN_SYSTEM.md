...
- `SelectionField` memakai button trigger + option buttons dengan `aria-expanded`/`aria-pressed`, Escape/outside-dismiss, optional search untuk taxonomy panjang, dan target sentuh minimal 44px. Pada `<=820px` daftar berkembang inline dalam form/sheet yang sama agar tidak membuat nested modal; desktop boleh memakai popover anchored.
...
- Buat Alokasi Dana mengikuti urutan mental user `Untuk apa → Ambil dana dari → Digunakan oleh → Dana awal → dana tersedia setelah dialokasikan`. Nominal awal baru ditampilkan setelah rekening sumber dipilih, dan copy menegaskan bahwa alokasi tidak memindahkan uang dari rekening.

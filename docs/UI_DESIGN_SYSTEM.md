...
- Rekening Investasi/RDN tidak memakai generic account reconciliation. Alert RDN membuka `/investasi` dan workflow Cocokkan Investasi untuk memverifikasi Cash RDN + holding.
...
- Pada Dashboard desktop, panel section `Rekening` boleh menjadi surface utama, tetapi setiap item rekening di dalam carousel **tidak boleh** menambah surface/card wrapper kedua di sekitar `AccountVisual`. Wrapper interaksi harus transparan tanpa border/background/shadow; shadow, hover, selected emphasis, dan keyboard focus diarahkan ke artwork kartu. Rekening non-investasi boleh menampilkan Saldo/Dana tersedia serta metadata langsung di bawah artwork; rekening Investasi memakai **Cash RDN** dan tidak memakai label Dana tersedia operasional. Beranda mobile tidak merender `AccountVisual`/kartu ATM; akses rekening disediakan melalui shortcut `Rekening` menuju `/rekening`.
...
- Halaman Anggaran memakai PageHeader desktop canonical; Accounts desktop menurunkan tinggi hero agar fakta rekening dan aktivitas lebih cepat terlihat tanpa mengubah presentation mobile.
...
- Copy canonical: **Saldo rekening** = cash non-investasi; **Aman digunakan** = dana operasional setelah proteksi/alokasi/komitmen; **Cash RDN** = uang tunai pada rekening investasi; **Nilai aset** = nilai holding; **Total investasi tercatat** = Nilai aset + Cash RDN; **Total kekayaan tercatat · saat ini** hanya untuk snapshot gabungan pada Laporan. Istilah generik summary memakai `aset`, bukan `saham`, agar saham dan reksa dana konsisten.
- Halaman Rekening tetap menampilkan RDN sebagai canonical account, tetapi presentation investment tidak memakai mental model `Dana tersedia/Dialokasikan`: kartu/summary menampilkan **Cash RDN** dan tujuan `Investasi`. Tren historis yang masih memuat cash seluruh rekening harus diberi label eksplisit **seluruh rekening termasuk Cash RDN** agar tidak disalahartikan sebagai Saldo rekening operasional.

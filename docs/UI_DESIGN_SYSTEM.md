...
- HTML native dan semantik diprioritaskan untuk input sederhana seperti text/date/checkbox. Pilihan app-owned yang membuka daftar tidak memakai native `<select>` browser; gunakan `SelectionField`/selection view canonical agar light/dark, touch, focus, dan hierarchy tetap konsisten. Toolkit digunakan hanya untuk perilaku kompleks yang memang belum dimiliki primitive project.
...
- Aksi user-facing memakai bahasa **Catat pembelian / Catat penjualan / Perbarui harga / Cocokkan** untuk menegaskan bahwa Saldo Bersama tidak mengeksekusi order broker. Aksi hanya tersedia untuk `portfolio.can_operate`; Koreksi catatan tetap Administrator-only. Jika saham aktif belum ada, CTA **Tambah saham** hanya boleh muncul untuk Administrator agar Member tidak masuk dead-end; holding inactive tetap boleh dijual/diberi harga sesuai capability backend. Capability frontend bukan security boundary dan dialog tetap mengirim mutation melalui API canonical dengan `row_version`/idempotency guard backend.
- Untuk prototype, **Tambah saham** tidak mengekspos form master `Ticker/Bursa/Nama/Lembar per lot`. User memilih dari katalog LQ45 lokal yang sengaja dibatasi (BBCA/BBRI/BMRI/TLKM/ASII/ICBP/ANTM); metadata `IDX` dan `100 lembar/lot` otomatis dari katalog. Setiap item memakai logo emiten WebP transparan di dalam surface 48px dengan fallback ticker mark untuk data legacy. Tidak ada tambah manual, market discovery, harga live, atau order action dari katalog. Daftar katalog adalah presentation scope dan tidak menggantikan backend instrument registry canonical.
...
- Mobile `<=820px` mempertahankan quick action minimum 44px, native text/date/month/textarea 16px saat memicu keyboard, app-owned selection minimum 44px, metadata finansial minimal 12px, single-column disclosure untuk nominal besar, dark/light token semantic, focus-visible, dan reduced-motion.

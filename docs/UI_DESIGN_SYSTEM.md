...
- Untuk route nested yang berbagi satu shell/header, trigger Info tetap satu tetapi isi wajib mengikuti sub-route aktif. `SettingsLayout` adalah pola canonical: `PageHeader` tetap satu, sedangkan help Notifikasi, Perangkat & sesi, Integrasi, Export/Import, Backup/Pemulihan, Reset, Periode, dan Audit berubah mengikuti `location.pathname`.
...
- `/pengaturan` adalah landing page grouped-list yang ringkas, bukan dashboard kartu. Halaman utama hanya menampilkan ringkasan akun/status backend dan tiga kelompok navigasi: **Umum**, **Data**, dan **Sistem**. Jangan merender seluruh menu Pengaturan lagi di atas setiap child route.
- Kelompok **Umum** berisi Notifikasi, Perangkat & sesi, dan Integrasi Google. Kelompok **Data** untuk Administrator berisi satu entry **Data & cadangan** serta **Pemeliharaan data**. Kelompok **Sistem** untuk Administrator berisi Periode & integritas serta Audit aktivitas.
...
- Nested route memakai header detail milik `SettingsLayout`: back-link kontekstual, satu heading route, dan satu Info trigger. Child page tidak membuat PageHeader kedua; heading child yang dipakai untuk `aria-labelledby` boleh tetap ada secara visually-hidden. Export/Import/Backup/Pemulihan kembali ke **Data & cadangan**, sedangkan detail lain kembali ke **Pengaturan**.
...
- Administrator-only item disembunyikan dari landing page Member, tetapi direct route tetap harus menampilkan guard dan backend selalu menjadi authorization utama.
- Tile layanan adalah `button` bila melakukan aksi. Navigasi memakai `Link`; status siap tanpa aksi tidak boleh diberi click handler pada elemen non-interaktif.

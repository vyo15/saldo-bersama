...
- Pada Dashboard desktop, panel section `Rekening` boleh menjadi surface utama, tetapi setiap item rekening di dalam carousel **tidak boleh** menambah surface/card wrapper kedua di sekitar `AccountVisual`. Wrapper interaksi harus transparan tanpa border/background/shadow; shadow, hover, selected emphasis, dan keyboard focus diarahkan ke artwork kartu. Saldo/Dana tersedia serta metadata boleh berada langsung di bawah artwork tanpa panel pemisah putih. Beranda mobile tidak merender `AccountVisual`/kartu ATM; akses rekening disediakan melalui shortcut `Rekening` menuju `/rekening`.
...
- Tepat di bawah carousel, mobile menampilkan satu summary finansial terpisah: **Dana tersedia** sebagai nominal utama, lalu tiga kolom compact **Saldo / Dialokasikan / Tersedia** dengan contextual help canonical dan privacy toggle. Aksi cepat di bawahnya adalah **Riwayat / Transfer / Kelola (atau Detail)** dalam satu baris; `Transfer` hanya ada di area ini agar header tidak memuat CTA ganda. `Transaksi terbaru` tampil sebagai surface informasi berikutnya. Grafik/analitik pengeluaran tidak ditempatkan di layar utama Rekening dan diarahkan ke Riwayat/route Transaksi yang memang memiliki presentation analitik canonical.

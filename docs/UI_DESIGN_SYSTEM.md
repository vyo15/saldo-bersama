...
- Login desktop memakai komposisi approved yang mengikuti reference-fit v7: copy manfaat yang compact di kiri, pasangan/ornamen keuangan transparan yang proporsional di tengah, dan panel autentikasi yang cukup lebar di kanan. Asset visual desktop disimpan terpisah di `public/login/assets/desktop/` (WebP transparan teroptimasi dari source exact-reference untuk karakter/ornamen dan SVG untuk badge Tujuan Bersama), sedangkan spacing, ukuran, urutan layer, typography, serta panel login dimiliki `LoginDesktopReference.module.css`. Artwork pasangan harus mempertahankan aspect ratio intrinsik (`height` terkontrol + `width:auto`) sehingga tidak tampak memanjang ke atas; headline/deskripsi/manfaat tidak boleh menyisakan gap vertikal berlebihan; panel login tidak boleh dikecilkan oleh wrapper tambahan dan tombol Google/trust indicator harus tetap menjadi kontrol DOM nyata. Tombol Google tetap tombol HTML branded yang sama dengan mobile dan transport/session authority tidak berubah. Mobile login tetap memiliki presentation sendiri dan tidak boleh ikut berubah hanya karena penyesuaian desktop.

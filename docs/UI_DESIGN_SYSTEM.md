...
- Rekening sumber tidak memakai `Cari rekening` atau `Lihat semua` di composer. Selector native hanya menampilkan rekening yang dapat dipakai untuk transaksi saat ini; Pengeluaran menyembunyikan rekening saldo Rp0, Transfer menyembunyikan sumber dengan dana tersedia Rp0, rekening `allow_negative` tetap valid, dan rekening yang sudah terpilih tetap dipertahankan agar edit transaksi lama tidak kehilangan referensi. Rekening tujuan tidak memakai filter saldo sumber ini.
- Metode pembayaran transaksi manual hanya menawarkan `Transfer`, `Tunai`, `Kartu debit`, dan `E-wallet` selain keadaan kosong. `Auto-debit` tidak dapat dipilih untuk transaksi manual baru; nilai legacy hanya dipertahankan secara tersembunyi saat mengedit data lama agar tidak berubah diam-diam.
...
- BCA, BNI, BTN, Mandiri, dan Permata memakai asset WebP 1536×968 sebagai base visual. Asset bank mempertahankan alpha/transparansi di luar siluet kartu, memuat wordmark/logo dan chip EMV satu kali, serta menyisakan safe area untuk overlay data rekening. Tunai, Tabungan, dan E-wallet memakai kanvas 1536×968 yang sama agar perceived size dan rasio konsisten. Wordmark/logo dekoratif dan chip bank berasal dari asset; HTML tidak boleh menggambarnya kembali.
...
- Gesture adalah enhancement, bukan satu-satunya jalur operasi. Swipe rekening/onboarding/bottom-sheet wajib memiliki jalur tombol/keyboard yang setara; mutation finansial/destructive tidak boleh hanya tersedia melalui gesture tersembunyi.
...
- Workflow yang sudah memiliki feedback lokal kaya tidak boleh menduplikasi GlobalProcessIndicator + toast. `transactions.create` dan `reconciliations.create` memakai result overlay lokal setelah server mengonfirmasi write; proses global untuk kedua action tersebut disupresi agar tidak muncul dua feedback sukses.
...
- Success result finansial mobile memakai full-screen celebration dengan MoneyRain staggered di belakang konten. Animasi wajib menghormati `prefers-reduced-motion`. Hasil rekonsiliasi difference tidak memakai celebration dan tetap mempertahankan warning yang dapat ditindaklanjuti.

...
- Route `/investasi` memakai visual portfolio modern di atas contract `investments.overview` yang sudah canonical: hero total nilai investasi, Cash RDN, nilai saham, modal saham, realized/unrealized P/L, quick action per portfolio, holding cards, komposisi nilai saat ini, dan aktivitas terbaru. Presentation tidak boleh membuat market discovery, harga live, atau performance time-series yang tidak tersedia dari backend.
...
- Aksi Beli/Jual/Harga/Cocokkan hanya tampil untuk `portfolio.can_operate`; Koreksi tetap hanya untuk Administrator. Capability frontend bukan security boundary dan dialog existing tetap mengirim mutation melalui API canonical dengan `row_version`/idempotency guard backend.

...
- Workflow lintas feature yang membawa route/state tetapi bukan mutation authority berada di `frontend/src/shared/workflows/`. Notification Center dan Dashboard berbagi `financialAlerts.js` untuk deep-link guidance serta `financialNotifications.js` untuk title/category/read-state UI; feature tidak boleh mengimpor `*Presentation.js` atau `*.api.js` milik feature lain.
- Route workflow yang membuka modal/aksi harus diperlakukan sebagai **one-shot intent**: identitas navigation (`location.key` + `workflowAction`) ditandai handled, route `state` dikonsumsi dengan `replace`, lalu UI dibuka. Effect tidak boleh bergantung pada wrapper resource/object hook yang berubah referensi tiap render bila hanya membutuhkan `status`/`data`; aturan ini mencegah render loop dan modal yang muncul kembali setelah `Batal`, `×`, Back, atau rerender resource.

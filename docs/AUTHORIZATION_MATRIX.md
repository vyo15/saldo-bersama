...
| `commitments.list` | Ya | Ya |
| `commitments.create` | Ya | Ya |
| `commitments.update` | Ya | Ya |
| `commitments.archive` | Ya | Tidak |
| `commitments.recordReceipt` | Ya | Ya |
| `budgets.list` | Ya | Ya |
...
- Kewajiban (domain internal `commitments`) mengikuti scope rekening sumber: Administrator/Member dapat create/update pada shared atau personal miliknya sendiri dan mencatat pembayaran/penerimaan yang kompatibel. Action internal `commitments.archive` tetap Administrator-only dan dipresentasikan sebagai **Hentikan kewajiban** di UI: record tidak lagi aktif, jadwal berikutnya dihentikan, sedangkan transaksi dan histori finansial tetap dipertahankan. Jadwal Rutin yang memiliki `commitment_id` hanya dapat diubah lifecycle-nya secara internal dari domain Kewajiban.

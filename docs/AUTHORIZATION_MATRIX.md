...
- Kewajiban (domain internal `commitments`) mengikuti scope rekening sumber: Administrator/Member dapat create/update pada shared atau personal miliknya sendiri dan mencatat pembayaran/penerimaan yang kompatibel. Action internal `commitments.archive` tetap Administrator-only dan dipresentasikan sebagai **Hapus** di UI agar histori finansial tidak di-hard-delete. Jadwal Rutin yang memiliki `commitment_id` hanya dapat diubah lifecycle-nya secara internal dari domain Kewajiban.

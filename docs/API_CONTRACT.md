...
| `envelopes.createWithNeeds` | Ya | Ya | Write/operation | Wajib | `api/_lib/services/planning/` |
...
| `notifications.updateSettings` | Ya | Ya | Write/operation | Wajib | `api/_lib/services/notifications.js` |
| `notifications.markRead` | Ya | Ya | Write/operation | Wajib | `api/_lib/services/notifications.js` |
...
- `commitments.create` membuat record domain internal Komitmen (label UI: **Kewajiban**) dan satu `recurring_rule` dalam scope rekening yang sama. Payload opsional `budget_id` menautkan Jadwal managed ke Kebutuhan kompatibel; relasi disimpan pada `recurring_rules.budget_id`, harus memiliki kategori/ownership/rekening sumber yang sama, dan ditolak atomik bila tidak kompatibel. `recurring_rules.commitment_id` menjadikan jadwal tersebut managed: update/archive langsung lewat action `recurring.*` ditolak agar tidak terjadi drift. Pembayaran historis sebelum create tidak direkonstruksi; `current_balance` adalah opening truth dan `installments_paid` boleh 0 untuk utang existing. Seluruh write baru memaksa metadata legacy `auto_debit=false`. Saat Kewajiban selesai, future occurrence reproducible dihentikan/diarsipkan internal; reversal pembayaran terakhir dapat mengaktifkan kembali schedule tanpa menghapus histori.
- `recurring.payOccurrence` pada Jadwal managed Kewajiban tetap membuat transaksi canonical. Untuk **KPR**, `remaining_principal` eksplisit adalah sumber authoritative untuk perubahan sisa pokok; bila tidak dikirim backend **tidak** menginfer pokok flat, mencatat `principal_known=0`, mempertahankan `current_balance`, tetapi tetap menambah `installments_paid` bila occurrence selesai. Untuk Cicilan/Pinjaman/Lainnya yang memiliki `original_amount + total_installments` valid, backend masih boleh menginfer pokok flat dari `round(original_amount/total_installments)` dan mencatat sisa sebagai bunga/biaya. `remaining_principal=0` digunakan untuk pelunasan final. Untuk Arisan, pembayaran mengurangi sisa setoran.
- Jadwal managed Kewajiban yang menaut ke Kebutuhan funded dapat dieksekusi scheduler mulai **tanggal jatuh tempo dan selama masih overdue** ketika Kebutuhan pada periode occurrence berhasil di-resolve, Alokasi terkait aktif, rekening sumber sama, serta sisa Kebutuhan dan Alokasi sama-sama cukup. Scheduler tidak melakukan partial debit, tidak memakai flag `auto_debit`, dan tetap memanggil `recurring.payOccurrence`; bila dana baru siap setelah jatuh tempo, occurrence overdue dapat dikejar pada run berikutnya. Untuk cicilan flat non-KPR terakhir, nominal otomatis dibatasi pada sisa pokok + bunga flat periode itu agar tidak overpay. Pada KPR, scheduler memakai nominal cicilan aktual dan tidak menebak penurunan pokok; rekonsiliasi sisa pokok dilakukan saat nilai bank aktual tersedia. Kegagalan kapasitas/saldo/duplicate membiarkan occurrence belum dibayar. Resolver `budget_id` mengikuti salinan Kebutuhan periode berjalan berdasarkan identity kategori+Alokasi+nama+scope agar link tidak putus setelah rollover. Scheduler juga memperbarui rolling projection horizon 24 bulan sekali per periode, sehingga KPR tenor panjang tidak mematerialisasi seluruh cicilan masa depan dan tetap memiliki occurrence setelah horizon awal bergeser.
...
- `notifications.center` mengembalikan event user-facing terbaru dari `notification_queue` yang hanya dimiliki actor saat ini beserta `readStates` actor dari `notification_read_states`. Action ini read-only, membatasi jumlah item/status baca, tidak mengekspos subscription endpoint/key, dan tidak mengubah status delivery.
...
- `notifications.preferences` mengembalikan tujuh tipe alert canonical untuk actor plus `settings` cadence. Belum adanya row preference berarti aktif kecuali `recurring_completed` yang default mati; setting default adalah rekonsiliasi 30 hari dan reminder konsistensi pencatatan mati. `notifications.updatePreference` hanya dapat mengubah preference milik actor sendiri, wajib `row_version` setelah row pertama tercipta, dan hanya memengaruhi alert terjadwal yang belum diantrikan; notification yang sudah masuk queue dapat tetap terkirim satu kali.
- `notifications.updateSettings` mengubah cadence actor sendiri dengan nilai canonical `reconciliation_days ∈ {0,14,30,60}` dan `recording_consistency_days ∈ {0,3,5,7}`. Write memakai `row_version`, audit server-side, dan tidak mengubah ledger/planning.
- `notifications.markRead` hanya menyimpan presentation state actor sendiri berupa pasangan `notification_key + fingerprint`; satu request dibatasi 120 identitas. Status **read** tidak menyelesaikan kondisi finansial: alert aktif tetap berasal dari source of truth dan fingerprint baru membuat kondisi yang berubah/memburuk muncul sebagai belum dibaca lagi.
...
- Pengaturan Notifikasi perangkat juga menyediakan **preview native lokal** untuk trial visual. Preview ini tidak memanggil `notifications.test`, tidak membuat queue/delivery backend, dan hanya menjalankan `ServiceWorkerRegistration.showNotification()` setelah izin perangkat diberikan. Copy dan asset preview bersifat demo non-sensitif; ukuran/crop rich image tetap ditentukan iOS/Android/browser dan tidak boleh dianggap contract layout absolut.
...
- `envelopes.createWithNeeds` adalah mutation canonical untuk flow **Alokasi baru**. Payload membawa field Alokasi yang sama dengan `envelopes.create`, `period_key`, dan array `needs` minimal satu item. Rule, periode awal, dan Kebutuhan dibuat dalam satu transaction; kegagalan validasi Kebutuhan/Jadwal tidak boleh meninggalkan Alokasi kosong. Pemanis (`decoration_key`) serta pilihan sisa periode (`rollover_policy=unallocated|carry`) tetap disimpan.
- Pendanaan otomatis Kebutuhan mengikat dana sebanyak Dana Tersedia. Jika kebutuhan lebih besar daripada dana bebas, mutation tetap sukses dan response funding membawa `requestedAmount`, `amount`, `availableAmount`, serta `shortageAmount`; saldo fisik/ledger tidak dibuat-buat. Pembayaran otomatis tetap wajib memeriksa dana aktual yang cukup.
- `recurring.createRule`/`recurring.updateRule` hanya menerima `default_account_id` non-investasi yang aktif dan dapat dioperasikan. Untuk expense, payload opsional `budget_id` boleh menautkan Jadwal Rutin ke Kebutuhan yang kategori, ownership, dan rekening sumber Alokasinya kompatibel; tepat satu kandidat dapat dipilih otomatis oleh UI, kandidat ambigu harus dipilih user, dan tanpa kandidat Jadwal tetap sah sebagai pembayaran mandiri. Jadwal yang tertaut tidak dicadangkan lagi pada `reservedBills` karena dananya sudah masuk Alokasi. Jadwal Rutin legacy yang menunjuk RDN tetap readable tetapi `can_pay=false`; frontend juga tidak menawarkan RDN pada selector pembayaran/rule baru.
- `budgets.upsert` mengizinkan Member membuat/mengubah Kebutuhan pada scope `shared` atau scope `personal` milik actor sendiri. `recording_mode` (`flexible|fixed_once|recurring`) boleh diubah hanya selama Kebutuhan belum memiliki pemakaian; setelah `used_amount > 0` mutation menolak perubahan dengan `BUDGET_RECORDING_MODE_LOCKED`. Perubahan dari `recurring` sebelum pemakaian meretire jadwal aktif yang terkait agar contract lama tidak tetap berjalan. `budgets.batchCreate` memakai boundary authorization yang sama untuk membuat maksimal 20 Kebutuhan dalam satu Alokasi/periode. Nama Kebutuhan wajib unik dalam periode/ownership/Alokasi yang sama sementara kategori master boleh dipakai oleh beberapa Kebutuhan; scope/owner harus sama dengan Alokasi, legacy budget tanpa `envelope_rule_id` membutuhkan `row_version`, dan Jadwal Rutin opsional yang dibuat bersama item harus memakai rekening sumber/ownership yang kompatibel; dispatcher menjalankan seluruh batch dalam satu transaction sehingga satu kegagalan me-rollback Kebutuhan serta jadwal lain di request tersebut. Bila Kebutuhan dihubungkan ke Alokasi Dana melalui `envelope_rule_id`, scope dan `owner_user_id` harus sama dengan Alokasi Dana tersebut. Scope personal milik pengguna lain tetap ditolak backend. Hak yang sama berlaku untuk Alokasi Dana dan Jadwal Rutin personal milik Member; Target baru tetap wajib memakai rekening Bersama.

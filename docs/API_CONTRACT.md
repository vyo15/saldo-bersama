...
- `investments.openingPositions.create` mencatat kondisi investasi yang sudah ada saat user mulai memakai aplikasi dan **bukan** transaksi Buy. Action menerima `portfolio_id`, `instrument_id`, `shares`, `cost_basis`, `reference_price`, `actual_cash`, optional `position_date`, serta optional `notes`. Event disimpan append-only sebagai semantic `opening_position`; `actual_cash` menghasilkan delta menuju Cash RDN aktual, reference price dapat menjadi fallback harga, dan beberapa posisi awal boleh ditambahkan selama fase opening masih terbuka. Fase opening ditutup setelah trade/valuation/reconciliation/correction reguler muncul; duplicate instrumen opening ditolak.
...
- Tren saldo harian/bulanan laporan tetap merepresentasikan **saldo cash seluruh rekening**, termasuk Cash RDN, dan wajib merekonsiliasi `investment_account_events` selain transaksi biasa. Snapshot **Total kekayaan tercatat · saat ini** di presentation dihitung sebagai `nonInvestmentBalance + investments.overview.summary.portfolio_value`; `totalBalance + portfolio_value` dilarang karena akan double-count Cash RDN. Historical market-value/net-worth tidak disintesis sebelum tersedia histori valuasi authoritative.

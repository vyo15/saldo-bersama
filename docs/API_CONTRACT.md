...
- Bila `reuse_needs=true` dan periode tujuan berada pada bulan berbeda, backend menyalin Kebutuhan aktif dari bulan sumber ke bulan tujuan memakai identitas kategori + ownership + `envelope_rule_id`. Kebutuhan yang sudah ada di periode tujuan selalu dipertahankan dan dilewati. Setelah copy, backend menghitung total Kebutuhan periode tujuan dan mendanai kekurangannya otomatis ke `allocated_amount` dari Dana Tersedia; carry yang sudah masuk dihitung sebagai pendanaan existing. Bila dana kurang, close/copy/funding di-rollback seluruhnya. Transaksi, saldo ledger, dan histori pemakaian tidak disalin. Response `envelopes.close` dapat mengembalikan `next_period`, `released_amount`, `rollover`, dan `needs_continuity`.
...
- `budgets.upsert` mengizinkan Member membuat/mengubah Kebutuhan pada scope `shared` atau scope `personal` milik actor sendiri. `budgets.batchCreate` memakai boundary authorization yang sama untuk membuat maksimal 20 Kebutuhan dalam satu Alokasi/periode. Batch wajib memiliki kategori unik, scope/owner harus sama dengan Alokasi, legacy budget tanpa `envelope_rule_id` membutuhkan `row_version`, dan Jadwal Rutin opsional harus memakai rekening sumber/ownership kompatibel. Untuk Kebutuhan ber-`envelope_rule_id`, backend menyesuaikan `envelope_periods.allocated_amount` otomatis: create/batch mendanai nominal baru, edit mendanai/melepas delta, archive melepas hanya bagian nominal yang belum terpakai (transaksi aktif pada Kebutuhan tetap dipertahankan sebagai komitmen historis), delete-unused melepas nominal penuh karena tidak memiliki histori, dan restore mendanai ulang hanya sisa yang belum terpakai. Edit tidak boleh menurunkan nominal di bawah pemakaian aktif dan mengembalikan `BUDGET_AMOUNT_BELOW_USED` (409) bila dilanggar. Pendanaan memvalidasi Dana Tersedia rekening sumber dan dapat mengembalikan `BUDGET_FUNDING_INSUFFICIENT` (409) dengan `requiredAmount`, `availableAmount`, `shortageAmount`, `accountBalance`, dan `allocatedRemaining`. Dispatcher menjalankan funding + Kebutuhan + Jadwal dalam satu transaction sehingga satu kegagalan me-rollback seluruh write; saldo ledger tidak berubah karena funding bukan transaksi. Scope personal milik pengguna lain tetap ditolak backend.

...
- Rekening `account_type=investment` bersifat **transfer-only pada ledger transaksi biasa**. `income`, `expense`, `refund`, dan `adjustment` baru yang menunjuk RDN ditolak backend dengan `INVESTMENT_ACCOUNT_TRANSFER_ONLY`; Buy/Sell/Koreksi tetap memakai event Investasi canonical. Data transaksi legacy tetap readable dan tidak dihapus otomatis.
- `investments.portfolios.create` menerima `rdn_account_id` yang operable actor **atau** `auto_create_rdn=true`. Mode otomatis membuat rekening canonical `account_type=investment`, saldo awal Rp0, `allow_negative=0`, dan menautkannya ke portfolio dalam transaksi database yang sama; Administrator memperoleh RDN `shared`, sedangkan Member memperoleh RDN `personal` milik actor. `source_label`/`name` hanya metadata manusia (mis. `Ajaib`) dan dapat dipakai sebagai qualifier nama RDN otomatis; `broker=ajaib|other` tetap metadata compatibility dan client canonical memakai `other`. Backend tetap menolak rekening non-investment, saldo-negatif-enabled, tidak aktif, tidak operable, atau RDN yang sudah terhubung. Response menambahkan `rdn_created_automatically`.
- `investments.overview` mengembalikan `portfolios[]`, `instruments[]`, dan `summary`. Setiap portfolio membawa Cash RDN, ownership/capability, holdings aktual, serta aktivitas terbaru. Holding membawa lot/share metadata, weighted `average_cost`, remaining `cost_basis`, `price_per_share`, `price_source=valuation|trade|opening_position`, `valuation_date`, `market_value`, realized/unrealized P/L. Aktivitas dapat berupa `trade`, `valuation`, `opening_position`, atau `correction` dan dipakai untuk rincian manual per saham; ini bukan feed broker/live market.
- Activity overview mempertahankan `activity_type=trade` + `trade_type=buy|sell` untuk kompatibilitas lama dan menambahkan `event_type=buy|sell|valuation|opening_position|correction` sebagai semantic event canonical.
...
- `investments.openingPositions.create` mencatat kondisi investasi yang sudah ada saat user mulai memakai aplikasi dan **bukan** transaksi Buy. Contract backend tetap menerima `portfolio_id`, `instrument_id`, `shares`, `cost_basis`, `reference_price`, optional `actual_cash`, optional `position_date`, serta optional `notes`. UI canonical meminta jumlah lot/unit + **harga rata-rata beli** + **harga sekarang**, lalu menghitung `cost_basis = shares × average_price` sebelum mutation; `reference_price` adalah harga/nilai sekarang. Bila `actual_cash` dihilangkan, Saldo RDN canonical dipertahankan (termasuk Rp0 pada RDN otomatis). Event disimpan append-only sebagai semantic `opening_position`; beberapa posisi awal boleh ditambahkan selama fase opening masih terbuka. Fase opening ditutup setelah trade/valuation/reconciliation/correction reguler muncul; duplicate instrumen opening ditolak.
- `investments.trades.buy` dan `investments.trades.sell` menerima `portfolio_id`, `instrument_id`, `lots`, `price_per_share`, `fee_amount`, optional `trade_date`, dan optional `notes` (maks. 500 karakter). Rupiah/lot/lembar integer, `share_quantity = lots × lot_size`, `gross_amount = share_quantity × price_per_share`, buy cash = gross + fee, sell cash = gross - fee. Trade tidak membuat row income/expense; cash RDN berasal dari view canonical `investment_account_events`. Buy ditolak bila RDN akan negatif, sell ditolak bila quantity melebihi holding.
...
- `reconciliations.create` hanya untuk rekening non-Investasi yang operable. Rekening `account_type=investment` ditolak dengan `INVESTMENT_RECONCILIATION_REQUIRED`; reconciliation RDN wajib memakai `investments.reconciliations.create` agar Cash RDN dan holding diverifikasi dalam satu checkpoint portfolio.
- `investments.corrections.create` Administrator-only, membutuhkan alasan, `row_version`, dan delta eksplisit. Koreksi tersimpan append-only dengan semantic `correction` dan tidak menulis ulang trade history; hasil holding/cost basis/cash negatif atau tidak konsisten ditolak. `opening_position` memakai action terpisah dan tidak dapat dipakai sebagai correction historis setelah fase opening ditutup.

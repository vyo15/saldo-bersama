...
| `investments.valuations.bulkUpdate` | Ya | Ya | Write/operation | Wajib | `api/_lib/services/investments.js` |
...
- `investments.valuations.update` membuat snapshot nilai append-only untuk satu aset dan tidak mengubah saldo, quantity, atau cost basis. `investments.valuations.bulkUpdate` menerima `valuation_date` + kelompok `portfolios[]` berisi `portfolio_id`, `row_version`, dan `valuations[{instrument_id,price_per_share}]`; seluruh batch divalidasi dan disimpan dalam satu transaksi database, lalu setiap portfolio hanya menaikkan `row_version` sekali. Saham memakai semantics harga per lembar, reksa dana memakai NAB per unit. Valuasi hanya boleh dicatat untuk aset yang benar-benar dimiliki pada `valuation_date` dan tidak boleh mendahului snapshot valuasi terbaru aset tersebut. Buy/Sell tetap tunduk pada checkpoint reconciliation legacy; semua mutation portfolio stale ditolak `CONFLICT`.
